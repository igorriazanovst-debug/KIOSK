...
Сверху — номер вопроса, чей ход, оставшееся время и текущие очки за вопрос. Сам вопрос — прямо над полем, а над текстом вопроса — тематическая картинка (своя для каждой из 6 тем банка: элементы, таблица Менделеева, строение атома, соединения, история химии, эксперименты). Картинка иллюстрирует ТЕМУ вопроса, а не сам ответ — это сознательное решение, чтобы иллюстрация не выдавала правильный ответ заранее. Кликните клетку, в которой, по-вашему, правильный ответ.

- **Очки убывают со временем** — чем быстрее ответили, тем больше получите за верный ответ.
- **Неверный клик** окрашивает клетку красным и показывает крестик ✗ (верный — зелёным и галочкой ✓, не только цветом — удобно, если цвета плохо различимы), засчитывает 0 очков за вопрос — ход переходит следующему игроку.
- **«Сдаюсь»** пропускает вопрос без клика, тоже 0 очков.
- Каждый вопрос без исключения можно кликнуть на любой видимой клетке поля — все они реагируют, не только несколько.
- Если у вопроса есть подсказка, кнопка «Показать подсказку» откроет её текст (сама подсказка заранее не видна). Подсказка есть у каждого вопроса встроенной викторины.
- Не ответили до конца таймера — вопрос засчитывается как неверный автоматически, партия не виснет.

**Расположение ответов на картинке меняется.** У встроенной викторины «Химия» есть три равнозначных варианта раскладки игрового поля — при каждом новом запуске программы случайно выбирается один из них. Если картинка выглядит немного по-другому, чем в прошлый раз, — это не ошибка, тексты вопросов и правильные ответы при этом всегда одни и те же.

При 2–3 игроках ход переходит по кругу — «Ходит: …» вверху всегда показывает, чья сейчас очередь.

**Две кнопки в шапке экрана:**

- **🔍 Крупнее** — увеличивает картинку и клетки, если текст ответов плохо виден. Повторное нажатие («Обычный размер») возвращает как было. Выбор запоминается и в следующей партии включать заново не нужно.
- **🔊 Звук** — короткий сигнал на верный/неверный ответ (восходящая трель или гудок). Нажмите, чтобы выключить, если мешает. Тоже запоминается между партиями.
