...
Заведите столько детей, сколько сядет за стол, — **до четырёх**. Затем отметьте тех, кто играет: галочка слева от имени. Отмеченные ходят по очереди, по кругу.

![Отмечены четверо — играют по очереди](img/words/04-игроки-четверо.png)
...
![Шестнадцать тем на выбор](img/words/05-темы.png)

В пособии **16 тем по 16 слов**: цифры и фигуры, комната, транспорт, еда, домашние и дикие животные, птицы, одежда, посуда, игрушки, фрукты и ягоды, овощи, школа, насекомые, природа, профессии. На карточке темы написано, сколько в ней слов, а если у темы уже есть награда — она показана здесь же.
...
Перед началом программа показывает **заставку темы**: её обложку и вводную часть. Вводная у каждой темы своя — она и написана, и звучит.

![Заставка темы: обложка и вводная часть](img/words/07-вводная.png)

---

## 4. Ход занятия

![Ход первого игрока — поле развёрнуто к нему](img/words/08-игра.png)

На экране:

- **сверху** — чей ход и звёздочки пройденных шагов;
- **в рамке** — силуэт загаданного предмета (на уровне Ⅰ);
- **под рамкой** — вопрос текстом: «Найди: Автобус»;
- **ниже** — карточки на выбор;
...
### Стол разворачивается к тому, чей ход

Дети сидят вокруг стола каждый со своей стороны, и читать экран «вверх ногами» никому не приходится: **при переходе хода всё содержимое разворачивается к следующему игроку** — и карточки, и вопрос, и имя ходящего, и кнопки.

Ход перешёл игроку слева — экран развернулся к нему:

![Поле развернулось к игроку слева](img/words/10-поворот-слева.png)

Ещё ход — и экран развёрнут к игроку напротив:

![Поле развёрнуто к игроку напротив](img/words/11-поворот-напротив.png)

У каждого места свой постоянный разворот: снизу без поворота, слева на четверть влево, сверху вверх ногами, справа на четверть вправо. Кто где сидит, показано на экране рассадки перед началом.

> **Кнопка «Выйти» разворачивается вместе со всем остальным** и оказывается в пределах досягаемости ребёнка. Если партию прерывают случайно — это она.
...
![Итоги и награды](img/words/12-итоги.png)
...
![Разделы педагога закрыты паролем](img/words/13-пароль.png)
...
![Настройки и уровни тем](img/words/14-настройки.png)
...
![Свои слова и комплекты](img/words/15-мои-слова.png)

Здесь заводятся собственные слова и собираются комплекты под конкретное занятие.

### Своё слово

![Создание своего слова](img/words/16-новое-слово.png)
