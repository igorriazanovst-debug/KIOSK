// Отрисовка подложки сцены в PNG — чтобы посмотреть на неё глазами.
//
// Рисует Chromium через CDP: тот же движок, что в приложении, поэтому видно
// ровно то, что увидит ученик. Проверка «наложений нет» этого не заменяет: она
// говорит про геометрию, а не про то, узнаётся ли предмет и не сливается ли он
// с фоном.
//
