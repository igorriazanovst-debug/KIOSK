...
// Импортируем всё из главного index
const shared = require('../../shared/dist/index.js');
const { generateLicenseKey, PLAN_CONFIGS, Plan } = shared;
...
async function main() {
  ...
  if (adminUser) {
    ...
    // Создаём через временное отключение constraint или используем транзакцию
    ...
    try {
      // Используем транзакцию для создания user и org
      await prisma.$transaction(async (tx) => {
        // 1. Создаём user БЕЗ organizationId (временно null)
        await tx.$executeRaw`
          INSERT INTO users (id, email, "passwordHash", role, "organizationId", "createdAt", "updatedAt")
          VALUES (${userId}, ${adminEmail}, ${passwordHash}, 'ADMIN', NULL, NOW(), NOW())
        `;
        
        // 2. Создаём organization с ownerUserId
        await tx.$executeRaw`
          INSERT INTO organizations (id, name, "ownerUserId", "createdAt", "updatedAt")
          VALUES (${orgId}, 'Demo Organization', ${userId}, NOW(), NOW())
        `;
        
        // 3. Обновляем user с organizationId
        await tx.$executeRaw`
          UPDATE users SET "organizationId" = ${orgId} WHERE id = ${userId}
        `;
      });
      ...
      console.error('Failed to create admin and organization:', error);
      throw error;
    }
  }
