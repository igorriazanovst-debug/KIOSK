...
export class AdminController {
  ...
  static async createLicense(req: Request, res: Response) {
    ...
    // Получить дефолтные значения seats из плана если не указаны
    const { getPlanFeatures } = await import('@kiosk/shared');
    const features = getPlanFeatures(plan.toLowerCase() as any);
    
    // Создать лицензию
    const license = await LicenseService.createLicense({
      organizationId,
      plan,
      seatsEditor: seatsEditor || features.maxEditorDevices,
      seatsPlayer: seatsPlayer || features.maxPlayerDevices,
      validFrom: new Date(),
