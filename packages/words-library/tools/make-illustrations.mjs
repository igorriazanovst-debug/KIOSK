...
// (wordImagePath/themeCoverPath в resources.ts), на неё опираются и проверка
// комплектности, и сверочные векторы, и контракт с будущей Android-реализацией.
// Менять расширение — значит править контракт для обеих платформ и
// перегенерировать векторы. Ради замены заглушек это несоразмерно.
//
...
// картинку. Когда появится настоящая векторная графика, файлы заменяются
// один в один, и обёртка просто исчезнет.
//
...
/** Итоговая сторона картинки: карточка в игре не больше 260 px, берём с запасом под Retina */
const TARGET = 512;

/** Рамка непрозрачной области — чтобы предмет занимал карточку целиком */
function contentBox(pngPath) {
  const raw = execFileSync(FF, ['-hide_banner', '-loglevel', 'error', '-i', pngPath,
    '-vf', 'alphaextract', '-f', 'rawvideo', '-pix_fmt', 'gray', '-'], { maxBuffer: 1 << 28 });
...
function makeOne(srcJpg, outSvg) {
  ...
  // Ключ и эрозия — те же параметры, что у совы: порог 0.30, выше нельзя,
  // иначе съедаются тёмные контуры рисунка
...
for (const file of fs.readdirSync(srcDir).filter((f) => f.endsWith('.jpg')).sort()) {
  ...
  const out = isTheme
    ? path.join(pkgRoot, 'assets', 'img', 'themes', `${id.slice('theme-'.length)}.svg`)
    : path.join(pkgRoot, 'assets', 'img', 'words', `${id}.svg`);
  try {
    const r = makeOne(path.join(srcDir, file), out);
    console.log(`${id} → ${path.relative(pkgRoot, out)} (${(r.bytes / 1024).toFixed(1)} КБ, кроп ${r.box})`);
    if (isTheme) themes += 1; else words += 1;
  } catch (err) {
...
console.log(`\nСлов: ${words}, тем: ${themes}, сбоев: ${failed}`);
