...
// Задания собираются ИЗ СЛОВАРЯ И ОПИСАНИЙ, а не держатся здесь списком.
// Зашитый список на двадцать четыре слова пришлось бы править при каждом
// изменении словаря, и он разошёлся бы с ним молча: генератор нарисовал бы
// слова, которых в пакете уже нет, и пропустил бы новые.
//
// Рассинхрон ловится ЗАРАНЕЕ: если у слова нет описания, прогон не
// начинается вовсе, а не выясняется это на сто пятидесятой картинке.
const { THEMES: CATALOGUE, buildLibrary } = await import('./catalogue.mjs');
const { WORD_PROMPTS, THEME_COVERS } = await import('./illustration-prompts.mjs');

const library = buildLibrary();
const promptOf = new Map();
for (const theme of CATALOGUE) {
  const block = WORD_PROMPTS[theme.id] ?? {};
  theme.words.forEach((name, i) => {
    promptOf.set(String(CATALOGUE.indexOf(theme) * 100 + i).padStart(4, '0'), block[name]);
  });
}

const ITEMS = library.words.map((w) => ({ id: w.id, what: promptOf.get(w.id) }));
const THEMES = library.themes.map((t) => ({ id: `theme-${t.id}`, what: THEME_COVERS[t.id] }));

const ALL = [...ITEMS, ...THEMES];

const noPrompt = ALL.filter((x) => !x.what);
if (noPrompt.length > 0) {
  console.error(`Без описания ${noPrompt.length}: ${noPrompt.slice(0, 8).map((x) => x.id).join(', ')}`);
  console.error('Прогон не начат: дописать illustration-prompts.mjs');
  process.exit(1);
}
