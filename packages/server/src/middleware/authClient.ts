...
/**
 * Middleware для аутентификации плеера по JWT токену.
 * Добавляет req.player = { licenseId, deviceId, plan }
 */
export const authenticatePlayer = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (payload.app !== 'player') {
      return res.status(403).json({ error: 'Player token required' });
    }
