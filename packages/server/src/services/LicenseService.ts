...
export class LicenseService {
  ...
  static async createLicense(params: {
    ...
    const planUpper = (typeof params.plan === 'string' 
      ? String(params.plan).toUpperCase() 
      : String(params.plan).toUpperCase()) as 'BASIC' | 'PRO' | 'MAX';
  ...
  static async updateLicense(
    ...
    if (updates.plan) {
      data.plan = (typeof updates.plan === 'string' 
        ? String(updates.plan).toUpperCase() 
        : String(updates.plan).toUpperCase()) as 'BASIC' | 'PRO' | 'MAX';
    }
    ...
    if (updates.status) {
      data.status = (typeof updates.status === 'string' 
        ...
        : String(updates.status).toUpperCase()) as 'ACTIVE' | 'SUSPENDED' | 'EXPIRED' | 'CANCELLED';
    }
