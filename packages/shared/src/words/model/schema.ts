...
export const LibraryThemeSchema = z.object({
  ...
  /**
   * Вводная часть темы (ТЗ строка 60). Своя у каждой темы, а не общий шаблон:
   * до 14.09.2026 вводная собиралась как «Тема: X. Послушай и найди нужную
   * картинку» и была одинаковой для всех тем — требование это закрывало лишь
   * по форме. Одинаковый текст вводной частью ТЕМЫ не является.
   *
   * Поле необязательное, чтобы старые пакеты контента продолжали читаться:
   * отсутствие вводной — это «пакет собран до появления поля», а не ошибка.
   * Экран заставки в этом случае показывает прежнюю шаблонную фразу.
   */
