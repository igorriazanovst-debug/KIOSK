...
/** Идентификатор поставочной сущности: транслитерация без начального «u» */
export const LibraryIdSchema = z
  .string()
  .regex(/^[a-z0-9_]{1,40}$/, 'идентификатор — строчная латиница, цифры и «_»')
  .refine((v) => !v.startsWith('u'), 'идентификатор поставочной сущности не начинается с «u»');

/** Идентификатор сущности, созданной педагогом */
export const UserIdSchema = z
  .string()
  .regex(/^u[a-f0-9]{16}$/, 'идентификатор своей сущности: «u» и 16 hex-символов');
...
export const LetterSchema = z.object({
  ...
  /**
   * Слова, начинающиеся с этой буквы. ТЗ (строка 71) требует не менее двух
   * иллюстраций на букву, а иллюстрация есть у каждого слова — значит
   * требование сводится к «не меньше двух слов у каждой буквы». Проверяется
   * отдельно, в checkLetterIllustrations: пустой список здесь допустим,
   * потому что промежуточное состояние пакета контента — законное.
   */
