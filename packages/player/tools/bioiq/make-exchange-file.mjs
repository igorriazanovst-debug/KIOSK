// packages/player/tools/bioiq/make-exchange-file.mjs
//
...
// docs/bioiq-acceptance-matrix.md §4.2 и в пункте 6.5 программы испытаний.
//
// Запуск: node --experimental-strip-types tools/bioiq/make-exchange-file.mjs [куда]

import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';

import { BioiqQuizSchema } from '../../src/bioiq/model/schema.ts';
import { checkBioiqQuiz } from '../../src/bioiq/model/completeness.ts';
import {
  buildExportPayload,
  serializeExportPayload,
  parseAndPersistImportedQuiz,
} from '../../src/bioiq/editor/quizExport.ts';

const HERE = path.dirname(fileURLToPath(import.meta.url));
const PLAYER = path.join(HERE, '..', '..');
const PUBLIC_BIOIQ = path.join(PLAYER, 'public', 'bioiq');
const OUT = process.argv[2] || path.join(PLAYER, '..', '..', 'docs', 'bioiq-exchange-check.json');

/** Сколько вопросов на уровень оставить: партия должна проходиться быстро. */
const PER_LEVEL = 4;

const real = JSON.parse(
  fs.readFileSync(path.join(PLAYER, 'src', 'bioiq', 'content', 'bioiqRealContent.json'), 'utf-8')
);
...
const quiz = BioiqQuizSchema.parse({
  ...real,
  id: 'bioiq-exchange-check',
...
const problems = checkBioiqQuiz(quiz);
...
// Картинки читаются с диска, а не через bioiqmedia:// — схема живёт только
...
const fetchAsBase64 = async (mediaUrl) => {
  ...
  const file = path.join(PUBLIC_BIOIQ, fileName);
