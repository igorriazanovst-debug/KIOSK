...
test('loadUserData returns a valid default when window.rusiqAPI is absent (non-Electron context)', async () => {
  ...
  assert.deepEqual(data, { schemaVersion: 1, sessions: [], soundOn: true, activeQuizId: null, teacherPinHash: null });
});

test('saveUserData does nothing (does not throw) when window.rusiqAPI is absent', () => {
  // @ts-expect-error
  delete globalThis.window;
  assert.doesNotThrow(() => saveUserData({ schemaVersion: 1, sessions: [], soundOn: true, activeQuizId: null, teacherPinHash: null }));
});
...
test('saveUserData then loadUserData round-trips the same data', async () => {
  ...
  const data = { schemaVersion: 1 as const, sessions: [{ id: 's1', quizId: 'q1', playedAtIso: '2026-01-01T00:00:00.000Z', players: [{ name: 'Аня', score: 10, correctCount: 1, totalCount: 1 }] }], soundOn: false, activeQuizId: null, teacherPinHash: null };
  saveUserData(data);
