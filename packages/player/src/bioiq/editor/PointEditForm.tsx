// packages/player/src/bioiq/editor/PointEditForm.tsx
...
import type { BioiqItemImageKind } from './quizStore.ts';

interface Props {
  question: BioiqQuestion;
  existingThemes: string[];
  onChange: (updated: BioiqQuestion) => void;
  onDelete: () => void;
  onClose: () => void;
  getImagePreviewUrl: (kind: BioiqItemImageKind) => string | null;
  onSelectImage: (kind: BioiqItemImageKind, file: File) => void;
  onRemoveImage: (kind: BioiqItemImageKind) => void;
}
...
// БиоIQ своя картинка на каждый уровень (план реализации §3) — уровень
...
const PointEditForm: React.FC<Props> = ({ question, existingThemes, onChange, onDelete, onClose, getImagePreviewUrl, onSelectImage, onRemoveImage }) => {
  ...
  function set<K extends keyof BioiqQuestion>(key: K, value: BioiqQuestion[K]) {
  ...
  return (
    <div className="ciq-card" style={{ marginTop: 16 }}>
      ...
      <label className="ciq-field">
        Тема
        <input value={question.theme} onChange={(e) => set('theme', e.target.value)} list="bioiq-editor-themes" className="ciq-input" />
        <datalist id="bioiq-editor-themes">
