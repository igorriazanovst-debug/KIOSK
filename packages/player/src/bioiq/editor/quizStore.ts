// packages/player/src/bioiq/editor/quizStore.ts
// Тонкая обёртка над window.bioiqAPI для каталога пользовательских
...
// схемой — только метаданные каталога; полная BioiqQuizSchema
// применяется к результату loadQuiz/перед saveQuiz.

import { BioiqQuizSchema, type BioiqQuiz } from '../model/schema.ts';
...
export type BioiqItemImageKind = 'question' | 'answer' | 'hint';

interface BioiqEditorAPI {
  listQuizzes: () => Promise<QuizListEntry[]>;
  loadQuiz: (quizId: string) => Promise<unknown>;
  saveQuiz: (quiz: BioiqQuiz) => Promise<{ ok: boolean }>;
...
function getEditorAPI(): BioiqEditorAPI | undefined {
  if (typeof window === 'undefined') return undefined;
  return (window as unknown as { bioiqAPI?: BioiqEditorAPI }).bioiqAPI;
}
...
export async function loadQuiz(quizId: string): Promise<BioiqQuiz | null> {
  const api = getEditorAPI();
  if (!api) return null;
  try {
    const raw = await api.loadQuiz(quizId);
    const parsed = BioiqQuizSchema.safeParse(raw);
...
export async function saveQuiz(quiz: BioiqQuiz): Promise<boolean> {
  const api = getEditorAPI();
  if (!api) return false;
  try {
