// packages/player/src/bioiq/editor/EditorScreen.tsx
...
// из 3 уровней сложности (план реализации Тип10_БиоIQ §3), не одна общая —
...
const LEVEL_TABS: { id: BioiqLevelId; label: string }[] = [
...
function pendingImageKey(questionId: string, kind: BioiqItemImageKind): string {
  return `${questionId}:${kind}`;
}

interface Props {
  initialQuiz: BioiqQuiz;
  ...
  onExit: (savedQuiz: BioiqQuiz | null) => void;
}
...
function makeBlankQuestion(point: BioiqPoint, level: BioiqLevelId): BioiqQuestion {
  return {
...
const EditorScreen: React.FC<Props> = ({ initialQuiz, pendingLevel1Image, onExit }) => {
  const [history, setHistory] = useState<History<BioiqQuiz>>(() => initHistory(initialQuiz));
  const [lastSavedQuiz, setLastSavedQuiz] = useState<BioiqQuiz | null>(pendingLevel1Image ? null : initialQuiz);
  const [pendingLevelImages, setPendingLevelImages] = useState<Partial<Record<BioiqLevelId, PendingLevelImage>>>(
    pendingLevel1Image
      ? { 1: { ...pendingLevel1Image, previewUrl: URL.createObjectURL(new Blob([pendingLevel1Image.buffer], { type: pendingLevel1Image.mimeType })) } }
      : {},
  );
  const [currentEditLevel, setCurrentEditLevel] = useState<BioiqLevelId>(1);
  ...
  function update(next: BioiqQuiz) {
  ...
  const levelImageUrl = levelPendingImage ? levelPendingImage.previewUrl : levelImageMeta ? bioiqLevelImageMediaUrl(levelImageMeta.fileName) : null;

  function handleAddQuestionPoint(point: BioiqPoint) {
  ...
  function handleAddDecoyToSelected(point: BioiqPoint) {
  ...
  function handleAddGenericDecoy(point: BioiqPoint) {
  ...
  function handleMoveDecoyOfQuestion(questionId: string, decoyIndex: number, point: BioiqPoint) {
    update({
      ...quiz,
  ...
  function handleMoveGenericDecoy(index: number, point: BioiqPoint) {
  ...
  function handleQuestionChange(updated: BioiqQuestion) {
    update({ ...quiz, questions: quiz.questions.map((q) => (q.id === updated.id ? updated : q)) });
  }

  async function handleSelectItemImage(questionId: string, kind: BioiqItemImageKind, file: File) {
  ...
  async function handleRemoveItemImage(questionId: string, kind: BioiqItemImageKind) {
    const key = pendingImageKey(questionId, kind);
    ...
    handleQuestionChange({ ...(question as BioiqQuestion), [field]: null });
  }

  function getItemImagePreviewUrl(question: BioiqQuestion, kind: BioiqItemImageKind): string | null {
    ...
    return fileName ? bioiqItemImageUrl(fileName) : null;
  }
  ...
  async function handleSelectLevelImage(level: BioiqLevelId, file: File) {
  ...
  async function handleSave() {
    ...
    const parsed = BioiqQuizSchema.safeParse(quiz);
    if (!parsed.success) {
      setSaveError(
    ...
    const pendingLevelKeys = Object.keys(pendingLevelImages) as unknown as BioiqLevelId[];
    ...
    if (pendingItemKeys.length > 0) {
      ...
      for (const key of pendingItemKeys) {
        const [questionId, kind] = key.split(':') as [string, BioiqItemImageKind];
  ...
  function handleLevelTabChange(level: BioiqLevelId) {
