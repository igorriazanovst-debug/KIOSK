// packages/player/src/bioiq/editor/QuizCatalogScreen.tsx
...
import { BIOIQ_QUIZ_SCHEMA_VERSION, type BioiqQuiz } from '../model/schema.ts';
import {
  buildExportPayload,
...
interface Props {
  ...
  onEditQuiz: (quiz: BioiqQuiz, pendingLevel1Image: { buffer: ArrayBuffer; mimeType: string } | null) => void;
...
async function buildBlankQuiz(result: NewQuizResult): Promise<{ quiz: BioiqQuiz; pendingLevel1Image: { buffer: ArrayBuffer; mimeType: string } }> {
...
const QuizCatalogScreen: React.FC<Props> = ({ builtinQuizTitle, activeQuizId, onSetActiveQuiz, onEditQuiz, onDuplicateBuiltin, onShowDailyStats, onExit }) => {
  ...
  async function performAction(quizId: string, action: PendingAction, titleForConfirm?: string) {
    ...
    if (action === 'duplicate') {
      ...
      const duplicated: BioiqQuiz = { ...quiz, id: newId, title: `${quiz.title} (копия)`, passwordHash: null };
