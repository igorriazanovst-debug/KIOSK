...
interface Props {
  ...
  onFinished: (answers: BioiqAnswerEvent[]) => void;
  soundOn: boolean;
  onSoundToggle: () => void;
  // Настройка «Крупнее» хранится в userData (BioiqRuntime) и переживает
...
const GameBoardScreen: React.FC<Props> = ({
  ...
  const [answers, setAnswers] = useState<BioiqAnswerEvent[]>([]);
  ...
  function commitAdvance(event: BioiqAnswerEvent) {
  ...
  function handleTileClick(tile: BioiqBoardTile) {
  ...
  function pointPosition(point: BioiqPoint): React.CSSProperties {
    return {
