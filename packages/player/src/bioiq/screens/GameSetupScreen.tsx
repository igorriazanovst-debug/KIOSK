// packages/player/src/bioiq/screens/GameSetupScreen.tsx
import React, { useState } from 'react';
import type { BioiqQuiz, BioiqLevelId } from '../model/schema.ts';
...
const GameSetupScreen: React.FC<Props> = ({ quiz, onComplete }) => {
  ...
  const [level, setLevel] = useState<BioiqLevelId>(1);
  ...
  function handleLevelChosen(chosenLevel: BioiqLevelId) {
  ...
  if (step === 'names') {
    ...
    // дошедшее до сохранения, ломает BioiqPlayerResultSchema.name (min(1))
