// packages/player/src/bioiq/dailyStats.ts
//
...
// только копился при завершении игры (BioiqRuntime.handleGameFinished)
...
export interface BioiqDailyPlayerStat {
  name: string;
  gamesPlayed: number;
  totalScore: number;
  totalCorrect: number;
...
export interface BioiqDailySummary {
  dateKey: string; // YYYY-MM-DD, локальная дата
  players: BioiqDailyPlayerStat[];
}
...
// рамках одной игры, отдельного профиля пользователя в БиоIQ нет).
...
export function aggregateSessionsByDay(sessions: BioiqSession[]): BioiqDailySummary[] {
  const byDate = new Map<string, Map<string, BioiqDailyPlayerStat>>();

  for (const session of sessions) {
    const dateKey = dateKeyOf(session.playedAtIso);
    if (!byDate.has(dateKey)) byDate.set(dateKey, new Map());
    const players = byDate.get(dateKey) as Map<string, BioiqDailyPlayerStat>;
