// packages/player/src/bioiq/model/completeness.test.ts
import { test } from 'node:test';
import assert from 'node:assert/strict';

import { BioiqQuizSchema, BIOIQ_QUIZ_SCHEMA_VERSION } from './schema.ts';
import {
  checkBioiqQuiz,
  rectsOverlap,
  BIOIQ_MIN_GENERIC_DECOYS_PER_LEVEL,
} from './completeness.ts';
import realContent from '../content/bioiqRealContent.json' with { type: 'json' };
...
function quizWith(overrides: Record<string, unknown> = {}) {
  const base = {
    ...
    genericDecoyPoints: [
      ...decoys(1, BIOIQ_MIN_GENERIC_DECOYS_PER_LEVEL),
      ...decoys(2, BIOIQ_MIN_GENERIC_DECOYS_PER_LEVEL),
      ...decoys(3, BIOIQ_MIN_GENERIC_DECOYS_PER_LEVEL),
    ],
    ...overrides,
  };
  return BioiqQuizSchema.parse(base);
}

test('полная викторина замечаний не вызывает', () => {
  assert.deepEqual(checkBioiqQuiz(quizWith()), []);
});

test('FR-013: девять точек без привязки на уровне — замечание с точным числом', () => {
  const quiz = quizWith({
    genericDecoyPoints: [
      ...decoys(1, BIOIQ_MIN_GENERIC_DECOYS_PER_LEVEL - 1),
      ...decoys(2, BIOIQ_MIN_GENERIC_DECOYS_PER_LEVEL),
      ...decoys(3, BIOIQ_MIN_GENERIC_DECOYS_PER_LEVEL),
    ],
  });
  const found = checkBioiqQuiz(quiz).filter((p) => p.requirement === 'FR-013');
...
test('FR-013: ровно десять точек — замечания нет, граница включительная', () => {
  const found = checkBioiqQuiz(quizWith()).filter((p) => p.requirement === 'FR-013');
...
test('FR-008: один и тот же вопрос на двух уровнях замечен', () => {
  ...
  const found = checkBioiqQuiz(quiz).filter((p) => p.requirement === 'FR-008');
  assert.equal(found.length, 1);
...
test('FR-008: повтор вопроса ВНУТРИ одного уровня замечанием не считается', () => {
  ...
  assert.deepEqual(checkBioiqQuiz(quiz).filter((p) => p.requirement === 'FR-008'), []);
});
...
test('налезающие друг на друга области названы поимённо', () => {
  ...
  const found = checkBioiqQuiz(quiz).filter((p) => p.message.includes('налезают'));
  assert.equal(found.length, 1);
...
test('FR-009: уровень без изображения-карты назван', () => {
  ...
  const found = checkBioiqQuiz(quiz);
