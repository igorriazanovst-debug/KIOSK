// packages/player/src/chimiq/dailyStats.ts
//
...
// только копился при завершении игры (ChimiqRuntime.handleGameFinished)
...
import type { ChimiqSession } from './model/schema.ts';

export interface ChimiqDailyPlayerStat {
  name: string;
  gamesPlayed: number;
  totalScore: number;
  totalCorrect: number;
...
export interface ChimiqDailySummary {
  dateKey: string; // YYYY-MM-DD, локальная дата
  players: ChimiqDailyPlayerStat[];
}
...
// рамках одной игры, отдельного профиля пользователя в ХимIQ нет).
...
export function aggregateSessionsByDay(sessions: ChimiqSession[]): ChimiqDailySummary[] {
  const byDate = new Map<string, Map<string, ChimiqDailyPlayerStat>>();

  for (const session of sessions) {
    const dateKey = dateKeyOf(session.playedAtIso);
    if (!byDate.has(dateKey)) byDate.set(dateKey, new Map());
    const players = byDate.get(dateKey) as Map<string, ChimiqDailyPlayerStat>;
