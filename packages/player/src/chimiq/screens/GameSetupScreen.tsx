// packages/player/src/chimiq/screens/GameSetupScreen.tsx
import React, { useState } from 'react';
import type { ChimiqQuiz, ChimiqLevelId } from '../model/schema.ts';
...
const GameSetupScreen: React.FC<Props> = ({ quiz, onComplete }) => {
  ...
  const [level, setLevel] = useState<ChimiqLevelId>(1);
  ...
  function handleLevelChosen(chosenLevel: ChimiqLevelId) {
  ...
  if (step === 'names') {
    ...
    // дошедшее до сохранения, ломает ChimiqPlayerResultSchema.name (min(1))
