// packages/player/src/chimiq/editor/SpecialCharPicker.tsx
...
// химии (`ХимIQ_3.1.1_разбор.md` §5, справка 4.4: «панель надстрочных и
...
const CHIMIQ_SPECIAL_CHARS = [...GENERAL_CHARS, ...SUBSCRIPT_DIGITS, ...SUPERSCRIPT_DIGITS, ...CHARGE_SIGNS, ...REACTION_ARROW];
...
const SpecialCharPicker: React.FC<Props> = ({ targetRef, onInsert }) => {
  ...
  return (
    <div role="group" aria-label="Вставить спецсимвол" style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 4 }}>
      {CHIMIQ_SPECIAL_CHARS.map((char, i) => (
        <button
          key={`${char}-${i}`}
          type="button"
          onClick={() => insert(char)}
