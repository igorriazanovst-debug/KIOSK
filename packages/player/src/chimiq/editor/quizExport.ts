// packages/player/src/chimiq/editor/quizExport.ts
...
// принят для rusiq (план реализации Тип9_ХимIQ §6, вопрос 2).
//
...
import { saveQuizLevelImage, saveQuizItemImage, type ChimiqItemImageKind } from './quizStore.ts';

export const CHIMIQ_EXPORT_FORMAT = 'chimiq-quiz-export-v1' as const;

export interface ChimiqExportedImage {
  base64: string;
  mimeType: string;
}

export interface ChimiqExportedFile {
  format: typeof CHIMIQ_EXPORT_FORMAT;
  quiz: ChimiqQuiz;
  ...
  // получателя.
  images: Record<string, ChimiqExportedImage>;
}
...
export type FetchAsBase64 = (mediaUrl: string) => Promise<ChimiqExportedImage | null>;
...
export async function fetchMediaAsBase64(mediaUrl: string): Promise<ChimiqExportedImage | null> {
  try {
...
export async function buildExportPayload(quiz: ChimiqQuiz, fetchAsBase64: FetchAsBase64): Promise<ChimiqExportedFile> {
  const images: Record<string, ChimiqExportedImage> = {};
  for (const levelKey of Object.keys(quiz.images)) {
    const meta = quiz.images[levelKey];
    await collectImage(images, fetchAsBase64, meta.fileName, chimiqLevelImageMediaUrl(meta.fileName));
  }
  for (const q of quiz.questions) {
    await collectImage(images, fetchAsBase64, q.questionImage, q.questionImage ? chimiqItemImageUrl(q.questionImage) : '');
    await collectImage(images, fetchAsBase64, q.answerImage, q.answerImage ? chimiqItemImageUrl(q.answerImage) : '');
    await collectImage(images, fetchAsBase64, q.hintImage, q.hintImage ? chimiqItemImageUrl(q.hintImage) : '');
  }
  return { format: CHIMIQ_EXPORT_FORMAT, quiz, images };
}

export function serializeExportPayload(payload: ChimiqExportedFile): string {
  return JSON.stringify(payload);
}

export function suggestExportFileName(quiz: ChimiqQuiz): string {
  const safeTitle = quiz.title.trim().replace(/[\\/:*?"<>|]/g, '_').slice(0, 80);
  return `${safeTitle.length > 0 ? safeTitle : 'Викторина'}.chimiq.json`;
}
...
export type ImportQuizResult = { ok: true; quiz: ChimiqQuiz } | { ok: false; error: string };

// Разбирает файл, полученный buildExportPayload/serializeExportPayload,
// обратно в валидный ChimiqQuiz со СВЕЖИМ id (та же причина, что у rusiq:
...
// не пройдёт ChimiqQuizSchema всё равно, лучше явная ошибка при импорте,
...
export async function parseAndPersistImportedQuiz(
  ...
  if (!isPlainRecord(parsed) || parsed.format !== CHIMIQ_EXPORT_FORMAT) {
    return { ok: false, error: 'Это не файл экспорта викторины ХимIQ' };
  }
  const quizResult = ChimiqQuizSchema.safeParse(parsed.quiz);
  ...
  const newImages: ChimiqQuiz['images'] = {};
  ...
  const newQuestions: ChimiqQuestion[] = [];
