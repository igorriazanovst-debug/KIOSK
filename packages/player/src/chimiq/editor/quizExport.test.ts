// packages/player/src/chimiq/editor/quizExport.test.ts
import { test } from 'node:test';
import assert from 'node:assert/strict';
import {
  CHIMIQ_EXPORT_FORMAT,
  buildExportPayload,
  serializeExportPayload,
  suggestExportFileName,
  parseAndPersistImportedQuiz,
  type ChimiqExportedImage,
  type FetchAsBase64,
  type PersistLevelImage,
  type PersistItemImage,
} from './quizExport.ts';
import { CHIMIQ_QUIZ_SCHEMA_VERSION, type ChimiqQuestion, type ChimiqQuiz } from '../model/schema.ts';

function validQuestion(overrides: Partial<ChimiqQuestion> = {}): ChimiqQuestion {
  return {
    id: 'q1',
    text: 'Вопрос',
    answer: 'Ответ',
...
function validQuiz(overrides: Partial<ChimiqQuiz> = {}): ChimiqQuiz {
  return {
...
const FAKE_IMAGE: ChimiqExportedImage = { base64: 'AAAA', mimeType: 'image/png' };
...
test('buildExportPayload collects all three level images keyed by their fileName', async () => {
  ...
  assert.equal(payload.format, CHIMIQ_EXPORT_FORMAT);
...
test('serializeExportPayload then JSON.parse round-trips the same structure', async () => {
  ...
  assert.equal(parsed.format, CHIMIQ_EXPORT_FORMAT);
...
test('suggestExportFileName sanitizes filesystem-unsafe characters and keeps Cyrillic', () => {
  ...
  assert.equal(suggestExportFileName(quiz), 'Тест_ _кавычки_ _ слэш_.chimiq.json');
});

test('suggestExportFileName falls back to a default name for a blank title', () => {
  const quiz = validQuiz({ title: '   ' });
  assert.equal(suggestExportFileName(quiz), 'Викторина.chimiq.json');
});
...
test('parseAndPersistImportedQuiz rejects a quiz payload that fails schema validation', async () => {
  const raw = JSON.stringify({ format: CHIMIQ_EXPORT_FORMAT, quiz: { title: 'incomplete' }, images: {} });
...
test('parseAndPersistImportedQuiz fails cleanly when a level image is missing from the file', async () => {
  const raw = JSON.stringify({ format: CHIMIQ_EXPORT_FORMAT, quiz: validQuiz(), images: {} });
  const result = await parseAndPersistImportedQuiz(raw, fakePersistLevelImage(), fakePersistItemImage());
  assert.equal(result.ok, false);
