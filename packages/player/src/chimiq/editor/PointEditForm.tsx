// packages/player/src/chimiq/editor/PointEditForm.tsx
...
import type { ChimiqItemImageKind } from './quizStore.ts';

interface Props {
  question: ChimiqQuestion;
  existingThemes: string[];
  onChange: (updated: ChimiqQuestion) => void;
  onDelete: () => void;
  onClose: () => void;
  getImagePreviewUrl: (kind: ChimiqItemImageKind) => string | null;
  onSelectImage: (kind: ChimiqItemImageKind, file: File) => void;
  onRemoveImage: (kind: ChimiqItemImageKind) => void;
}
...
// ХимIQ своя картинка на каждый уровень (план реализации §3) — уровень
...
const PointEditForm: React.FC<Props> = ({ question, existingThemes, onChange, onDelete, onClose, getImagePreviewUrl, onSelectImage, onRemoveImage }) => {
  ...
  function set<K extends keyof ChimiqQuestion>(key: K, value: ChimiqQuestion[K]) {
  ...
  return (
    <div className="ciq-card" style={{ marginTop: 16 }}>
      ...
      <label className="ciq-field">
        Тема
        <input value={question.theme} onChange={(e) => set('theme', e.target.value)} list="chimiq-editor-themes" className="ciq-input" />
        <datalist id="chimiq-editor-themes">
