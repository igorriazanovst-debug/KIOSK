// packages/player/src/chimiq/editor/EditorScreen.tsx
...
// из 3 уровней сложности (план реализации Тип9_ХимIQ §3), не одна общая —
...
import { saveQuiz, saveQuizLevelImage, saveQuizItemImage, deleteQuizItemImage, type ChimiqItemImageKind } from './quizStore.ts';
import { chimiqLevelImageMediaUrl, chimiqItemImageUrl } from '../chimiqMediaUrl.ts';
import { ChimiqQuizSchema, CHIMIQ_DEFAULT_POINT_SIZE, type ChimiqLevelId, type ChimiqPoint, type ChimiqQuestion, type ChimiqQuiz } from '../model/schema.ts';
import '../chimiqTheme.css';

const ITEM_IMAGE_FIELD_BY_KIND: Record<ChimiqItemImageKind, 'questionImage' | 'answerImage' | 'hintImage'> = {
  question: 'questionImage',
  answer: 'answerImage',
  hint: 'hintImage',
};

const LEVEL_TABS: { id: ChimiqLevelId; label: string }[] = [
...
function pendingImageKey(questionId: string, kind: ChimiqItemImageKind): string {
  return `${questionId}:${kind}`;
}

interface Props {
  initialQuiz: ChimiqQuiz;
  ...
  onExit: (savedQuiz: ChimiqQuiz | null) => void;
}
...
function makeBlankQuestion(point: ChimiqPoint, level: ChimiqLevelId): ChimiqQuestion {
  return {
    ...
    x: point.x,
    y: point.y,
    width: CHIMIQ_DEFAULT_POINT_SIZE,
    height: CHIMIQ_DEFAULT_POINT_SIZE,
    decoyPoints: [],
    price: 100,
    timeSeconds: 30,
    level,
...
const EditorScreen: React.FC<Props> = ({ initialQuiz, pendingLevel1Image, onExit }) => {
  const [history, setHistory] = useState<History<ChimiqQuiz>>(() => initHistory(initialQuiz));
  const [lastSavedQuiz, setLastSavedQuiz] = useState<ChimiqQuiz | null>(pendingLevel1Image ? null : initialQuiz);
  const [pendingLevelImages, setPendingLevelImages] = useState<Partial<Record<ChimiqLevelId, PendingLevelImage>>>(
    pendingLevel1Image
      ? { 1: { ...pendingLevel1Image, previewUrl: URL.createObjectURL(new Blob([pendingLevel1Image.buffer], { type: pendingLevel1Image.mimeType })) } }
      : {},
  );
  const [currentEditLevel, setCurrentEditLevel] = useState<ChimiqLevelId>(1);
  ...
  function update(next: ChimiqQuiz) {
  ...
  const levelImageUrl = levelPendingImage ? levelPendingImage.previewUrl : levelImageMeta ? chimiqLevelImageMediaUrl(levelImageMeta.fileName) : null;

  function handleAddQuestionPoint(point: ChimiqPoint) {
  ...
  function handleAddDecoyToSelected(point: ChimiqPoint) {
  ...
  function handleAddGenericDecoy(point: ChimiqPoint) {
  ...
  function handleMoveDecoyOfQuestion(questionId: string, decoyIndex: number, point: ChimiqPoint) {
    update({
      ...quiz,
  ...
  function handleMoveGenericDecoy(index: number, point: ChimiqPoint) {
  ...
  function handleQuestionChange(updated: ChimiqQuestion) {
    update({ ...quiz, questions: quiz.questions.map((q) => (q.id === updated.id ? updated : q)) });
  }

  async function handleSelectItemImage(questionId: string, kind: ChimiqItemImageKind, file: File) {
  ...
  async function handleRemoveItemImage(questionId: string, kind: ChimiqItemImageKind) {
    const key = pendingImageKey(questionId, kind);
    ...
    handleQuestionChange({ ...(question as ChimiqQuestion), [field]: null });
  }

  function getItemImagePreviewUrl(question: ChimiqQuestion, kind: ChimiqItemImageKind): string | null {
    ...
    return fileName ? chimiqItemImageUrl(fileName) : null;
  }
  ...
  async function handleSelectLevelImage(level: ChimiqLevelId, file: File) {
  ...
  async function handleSave() {
    ...
    const parsed = ChimiqQuizSchema.safeParse(quiz);
    if (!parsed.success) {
      setSaveError(
    ...
    const pendingLevelKeys = Object.keys(pendingLevelImages) as unknown as ChimiqLevelId[];
    ...
    if (pendingItemKeys.length > 0) {
      ...
      for (const key of pendingItemKeys) {
        const [questionId, kind] = key.split(':') as [string, ChimiqItemImageKind];
  ...
  function handleLevelTabChange(level: ChimiqLevelId) {
