// packages/player/src/chimiq/thematicImages.ts
//
...
// content/chimiqRealContent.json — те решают другую задачу, кликабельные
...
export interface ChimiqThematicImage {
  id: string;
  fileName: string;
  title: string;
  caption: string;
...
export const CHIMIQ_THEMATIC_IMAGES: ChimiqThematicImage[] = [
  {
    id: 'periodic-table',
...
export function chimiqThematicImageUrl(fileName: string): string {
  return `./chimiq/thematic/${fileName}`;
}
