// zod-схемы данных виджета «ХимIQ» (Тип 9) — единственная точка, через
...
// реализации `Тип9_ХимIQ/Тип9_план_реализации.md` §1.
//
...
// конкретной картинке).
export const CHIMIQ_DEFAULT_POINT_SIZE = 100;

export const ChimiqPointSchema = z.object({
  x: z.number(),
  y: z.number(),
  width: z.number().positive().default(CHIMIQ_DEFAULT_POINT_SIZE),
  height: z.number().positive().default(CHIMIQ_DEFAULT_POINT_SIZE),
});
export type ChimiqPoint = z.infer<typeof ChimiqPointSchema>;

export const ChimiqLevelIdSchema = z.union([z.literal(1), z.literal(2), z.literal(3)]);
export type ChimiqLevelId = z.infer<typeof ChimiqLevelIdSchema>;

export const ChimiqQuestionSchema = z.object({
  id: z.string().min(1),
  text: z.string().min(1),
  ...
  width: z.number().positive().default(CHIMIQ_DEFAULT_POINT_SIZE),
  height: z.number().positive().default(CHIMIQ_DEFAULT_POINT_SIZE),
  decoyPoints: z.array(ChimiqPointSchema).default([]),
  price: z.number().int().positive(),
  timeSeconds: z.number().int().positive(),
  level: ChimiqLevelIdSchema,
...
export type ChimiqQuestion = z.infer<typeof ChimiqQuestionSchema>;

export const ChimiqLevelSchema = z.object({
  id: ChimiqLevelIdSchema,
  label: z.string().min(1),
});
export type ChimiqLevel = z.infer<typeof ChimiqLevelSchema>;
...
// у ХимIQ три изображения — поэтому у точки этого типа ЕСТЬ level, а у
// decoyPoints внутри question.decoyPoints level не нужен: он уже задан
// уровнем самого вопроса-владельца.
export const ChimiqGenericDecoyPointSchema = ChimiqPointSchema.extend({
  level: ChimiqLevelIdSchema,
});
export type ChimiqGenericDecoyPoint = z.infer<typeof ChimiqGenericDecoyPointSchema>;
...
export const ChimiqLevelImageSchema = z.object({
  fileName: z.string().min(1),
  width: z.number().int().positive(),
  height: z.number().int().positive(),
});
export type ChimiqLevelImage = z.infer<typeof ChimiqLevelImageSchema>;

export const CHIMIQ_QUIZ_SCHEMA_VERSION = 1 as const;

const ChimiqQuizShapeSchema = z.object({
  schemaVersion: z.literal(CHIMIQ_QUIZ_SCHEMA_VERSION),
  id: z.string().min(1),
  title: z.string().min(1),
  intro: z.string().default(''),
  themes: z.array(z.string().min(1)).default([]),
  passwordHash: z.string().nullable().default(null),
  // КЛЮЧЕВОЕ ОТЛИЧИЕ ОТ RUSIQ: у ХимIQ своё изображение-карта на КАЖДЫЙ
  ...
  // что ChimiqLevelIdSchema (1|2|3), но zod-объект с числовыми ключами
  // не даёт статической полноты — полнота (все три уровня присутствуют)
  // проверяется в superRefine ниже вместе с bounds.
  images: z.record(z.string(), ChimiqLevelImageSchema),
  levels: z.array(ChimiqLevelSchema).min(1),
  questions: z.array(ChimiqQuestionSchema).min(1),
  genericDecoyPoints: z.array(ChimiqGenericDecoyPointSchema).default([]),
});
...
export const ChimiqQuizSchema = ChimiqQuizShapeSchema.superRefine((quiz, ctx) => {
...
export type ChimiqQuiz = z.infer<typeof ChimiqQuizShapeSchema>;

// ─── Пользовательские данные — отдельно от контента ────────────────────────

export const ChimiqPlayerResultSchema = z.object({
  name: z.string().min(1),
  score: z.number().int().nonnegative(),
  correctCount: z.number().int().nonnegative(),
  totalCount: z.number().int().nonnegative(),
});
export type ChimiqPlayerResult = z.infer<typeof ChimiqPlayerResultSchema>;

export const ChimiqSessionSchema = z.object({
  id: z.string().min(1),
  quizId: z.string().min(1),
  playedAtIso: z.string().min(1),
  players: z.array(ChimiqPlayerResultSchema).min(1),
});
export type ChimiqSession = z.infer<typeof ChimiqSessionSchema>;

export const CHIMIQ_USERDATA_SCHEMA_VERSION = 1 as const;

export const ChimiqUserDataSchema = z.object({
...
export type ChimiqUserData = z.infer<typeof ChimiqUserDataSchema>;
