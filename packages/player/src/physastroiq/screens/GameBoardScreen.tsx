...
interface Props {
  ...
  imageHeight: number;
  playerNames: string[];
  questionsByPlayer: PhysastroiqQuestion[][];
  levelQuestions: PhysastroiqQuestion[];
  genericDecoyPoints: PhysastroiqPoint[];
  onFinished: (answers: PhysastroiqAnswerEvent[]) => void;
  soundOn: boolean;
  onSoundToggle: () => void;
  // Настройка «Крупнее» хранится в userData (PhysastroiqRuntime) и переживает
...
const GameBoardScreen: React.FC<Props> = ({
  ...
  const [answers, setAnswers] = useState<PhysastroiqAnswerEvent[]>([]);
  ...
  function commitAdvance(event: PhysastroiqAnswerEvent) {
  ...
  function handleTileClick(tile: PhysastroiqBoardTile) {
  ...
  function pointPosition(point: PhysastroiqPoint): React.CSSProperties {
    return {
