// zod-схемы данных виджета «ФизАстроIQ» (Тип 11) — единственная точка, через
...
// реализации `Тип11_ФизАстроIQ/ФизАстро_план_реализации.md` §1.
//
...
// конкретной картинке).
export const PHYSASTROIQ_DEFAULT_POINT_SIZE = 100;

export const PhysastroiqPointSchema = z.object({
  x: z.number(),
  y: z.number(),
  width: z.number().positive().default(PHYSASTROIQ_DEFAULT_POINT_SIZE),
  height: z.number().positive().default(PHYSASTROIQ_DEFAULT_POINT_SIZE),
});
export type PhysastroiqPoint = z.infer<typeof PhysastroiqPointSchema>;

export const PhysastroiqLevelIdSchema = z.union([z.literal(1), z.literal(2), z.literal(3)]);
export type PhysastroiqLevelId = z.infer<typeof PhysastroiqLevelIdSchema>;

export const PhysastroiqQuestionSchema = z.object({
  id: z.string().min(1),
  text: z.string().min(1),
  ...
  width: z.number().positive().default(PHYSASTROIQ_DEFAULT_POINT_SIZE),
  height: z.number().positive().default(PHYSASTROIQ_DEFAULT_POINT_SIZE),
  decoyPoints: z.array(PhysastroiqPointSchema).default([]),
  price: z.number().int().positive(),
  timeSeconds: z.number().int().positive(),
  level: PhysastroiqLevelIdSchema,
...
export type PhysastroiqQuestion = z.infer<typeof PhysastroiqQuestionSchema>;

export const PhysastroiqLevelSchema = z.object({
  id: PhysastroiqLevelIdSchema,
  label: z.string().min(1),
});
export type PhysastroiqLevel = z.infer<typeof PhysastroiqLevelSchema>;
...
// у ФизАстроIQ три изображения — поэтому у точки этого типа ЕСТЬ level, а у
// decoyPoints внутри question.decoyPoints level не нужен: он уже задан
// уровнем самого вопроса-владельца.
export const PhysastroiqGenericDecoyPointSchema = PhysastroiqPointSchema.extend({
  level: PhysastroiqLevelIdSchema,
});
export type PhysastroiqGenericDecoyPoint = z.infer<typeof PhysastroiqGenericDecoyPointSchema>;
...
export const PhysastroiqLevelImageSchema = z.object({
  fileName: z.string().min(1),
  width: z.number().int().positive(),
  height: z.number().int().positive(),
});
export type PhysastroiqLevelImage = z.infer<typeof PhysastroiqLevelImageSchema>;

export const PHYSASTROIQ_QUIZ_SCHEMA_VERSION = 1 as const;

const PhysastroiqQuizShapeSchema = z.object({
  schemaVersion: z.literal(PHYSASTROIQ_QUIZ_SCHEMA_VERSION),
  id: z.string().min(1),
  title: z.string().min(1),
  intro: z.string().default(''),
  themes: z.array(z.string().min(1)).default([]),
  passwordHash: z.string().nullable().default(null),
  // КЛЮЧЕВОЕ ОТЛИЧИЕ ОТ RUSIQ: у ФизАстроIQ своё изображение-карта на КАЖДЫЙ
  ...
  // что PhysastroiqLevelIdSchema (1|2|3), но zod-объект с числовыми ключами
  // не даёт статической полноты — полнота (все три уровня присутствуют)
  // проверяется в superRefine ниже вместе с bounds.
  images: z.record(z.string(), PhysastroiqLevelImageSchema),
  levels: z.array(PhysastroiqLevelSchema).min(1),
  questions: z.array(PhysastroiqQuestionSchema).min(1),
  genericDecoyPoints: z.array(PhysastroiqGenericDecoyPointSchema).default([]),
});
...
export const PhysastroiqQuizSchema = PhysastroiqQuizShapeSchema.superRefine((quiz, ctx) => {
...
export type PhysastroiqQuiz = z.infer<typeof PhysastroiqQuizShapeSchema>;

// ─── Пользовательские данные — отдельно от контента ────────────────────────

export const PhysastroiqPlayerResultSchema = z.object({
  name: z.string().min(1),
  score: z.number().int().nonnegative(),
  correctCount: z.number().int().nonnegative(),
  totalCount: z.number().int().nonnegative(),
});
export type PhysastroiqPlayerResult = z.infer<typeof PhysastroiqPlayerResultSchema>;

export const PhysastroiqSessionSchema = z.object({
  id: z.string().min(1),
  quizId: z.string().min(1),
  playedAtIso: z.string().min(1),
  players: z.array(PhysastroiqPlayerResultSchema).min(1),
});
export type PhysastroiqSession = z.infer<typeof PhysastroiqSessionSchema>;

export const PHYSASTROIQ_USERDATA_SCHEMA_VERSION = 1 as const;

export const PhysastroiqUserDataSchema = z.object({
  schemaVersion: z.literal(PHYSASTROIQ_USERDATA_SCHEMA_VERSION),
  sessions: z.array(PhysastroiqSessionSchema).default([]),
...
export type PhysastroiqUserData = z.infer<typeof PhysastroiqUserDataSchema>;
