// packages/player/src/physastroiq/model/completeness.ts
//
...
//
// FR-013 (ТЗ строка 334) — «...не менее 10 координат точек без привязки к ним
...
// FR-008 (строка 329) — «каждый из уровней должен иметь УНИКАЛЬНЫЙ набор
//   вопросов». Один и тот же вопрос на двух уровнях — не «сложнее», а тот же
//   вопрос дважды.
// FR-009 (строка 330) — изображение ко всем вопросам: у каждого уровня должна
//   быть своя карта.
//
...
// рядом с амперметром, фокус на самой оптической оси. Две наложенные
// области дают кнопку
...
import type { PhysastroiqQuiz, PhysastroiqLevelId } from './schema';

/** ТЗ 11, FR-013 (строка 334): «не менее 10 координат точек без привязки». */
export const PHYSASTROIQ_MIN_GENERIC_DECOYS_PER_LEVEL = 10;

export interface PhysastroiqQuizProblem {
...
const LEVELS: PhysastroiqLevelId[] = [1, 2, 3];

export function checkPhysastroiqQuiz(quiz: PhysastroiqQuiz): PhysastroiqQuizProblem[] {
  const problems: PhysastroiqQuizProblem[] = [];
  ...
  for (const level of LEVELS) {
    ...
    if (levelDecoys.length < PHYSASTROIQ_MIN_GENERIC_DECOYS_PER_LEVEL) {
      problems.push({
        requirement: 'FR-013',
        message:
          `уровень ${level}: точек без привязки к вопросу ${levelDecoys.length}, ` +
          `требуется не меньше ${PHYSASTROIQ_MIN_GENERIC_DECOYS_PER_LEVEL}`,
      });
    }
  ...
  const seen = new Map<string, PhysastroiqLevelId>();
