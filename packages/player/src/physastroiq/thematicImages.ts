...
//
// FR-022 ТЗ 11 (строка 343): «количество тематических изображений по биологии
// ≥ 3 шт». FR-023 (строка 344): «темы изображений: внутренние органы,
// растения, клетки» — три темы названы дословно, и на приёмке их будут искать
// именно так.
//
// ПОЧЕМУ ШЕСТЬ, А НЕ ТРИ. У эталона (ОС3. ФизАстро IQ 3.1) изображений ровно три
...
export type PhysastroiqImageTheme = 'organs' | 'plants' | 'cells';

export const PHYSASTROIQ_REQUIRED_THEMES: { id: PhysastroiqImageTheme; russian: string }[] = [
...
export function missingThematicThemes(): string[] {
  return PHYSASTROIQ_REQUIRED_THEMES.filter(
    (t) => !PHYSASTROIQ_THEMATIC_IMAGES.some((img) => img.theme === t.id)
  ).map((t) => t.russian);
}
