...
test('есть микро — размеры клеток измеряются в мкм', () => {
  assert.ok(PHYSASTROIQ_SPECIAL_CHARS.includes('μ'));
});
...
test('индексы и заряды СОХРАНЕНЫ, а не выброшены вместе с химией', () => {
  ...
  for (const ch of ['⁺', '⁻', '²']) {
    assert.ok(PHYSASTROIQ_SPECIAL_CHARS.includes(ch), `нет знака заряда ${ch}`);
  }
  assert.ok(PHYSASTROIQ_SPECIAL_CHARS.includes('→'), 'нет стрелки — нужна и реакции, и цепи питания');
});
...
test('полный греческий алфавит в набор не попал', () => {
  ...
  assert.ok(PHYSASTROIQ_SPECIAL_CHARS.includes('α'));
  assert.ok(PHYSASTROIQ_SPECIAL_CHARS.includes('β'));
