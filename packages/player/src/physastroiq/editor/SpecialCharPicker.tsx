// packages/player/src/physastroiq/editor/SpecialCharPicker.tsx
// FR-015 ТЗ 11 (строка 336) — «вставить в текст вопроса, ответа или подсказки
...
const SpecialCharPicker: React.FC<Props> = ({ targetRef, onInsert }) => {
  ...
  return (
    <div role="group" aria-label="Вставить спецсимвол" style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 4 }}>
      {PHYSASTROIQ_SPECIAL_CHARS.map((char, i) => (
        <button
          key={`${char}-${i}`}
          type="button"
          onClick={() => insert(char)}
