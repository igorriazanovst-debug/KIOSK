// packages/player/src/physastroiq/content/physastroiqRealContent.test.ts
//
...
import realContent from './physastroiqRealContent.json' with { type: 'json' };
...
test('викторина проходит схему', () => {
  PhysastroiqQuizSchema.parse(realContent);
});

test('викторина проходит проверку комплектности', () => {
  assert.deepEqual(checkPhysastroiqQuiz(PhysastroiqQuizSchema.parse(realContent)), []);
});
