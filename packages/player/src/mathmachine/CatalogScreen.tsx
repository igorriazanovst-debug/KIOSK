...
import { listGroupsForTopic, tasksForGroup, nextUndoneTaskId, summarizeGroupProgress } from '@kiosk/shared';
import TaskRunner from './TaskRunner';
import Mascot from './Mascot';
import { listTopicsByCategory, isTopicComplete } from './topicCategories';
...
const CatalogScreen: React.FC<Props> = ({ content, progress, soundOn, onProgressChange }) => {
  ...
  const categories = listTopicsByCategory(content);
    ...
    <div style={{ padding: '24px 32px 48px', fontFamily: FONT.ui, color: COLOR.text }}>
      <div style={greetingRowStyle}>
        ...
        <div>
          ...
          <div style={{ color: COLOR.textMuted, fontSize: 15 }}>Матвей поможет — просто нажми на раздел</div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 20 }}>
        {categories.map((category) => {
          ...
          const doneTopics = category.topics.filter((t) => isTopicComplete(content, progress, t.topicId)).length;
          return (
            <div key={category.key} style={categorySectionStyle}>
              <button
                onClick={() => {
                  setOpenCategoryKey(isOpen ? null : category.key);
                  setOpenTopicId(null);
                }}
                style={categoryHeaderStyle(isOpen)}
              >
                <span style={{ flex: 1, textAlign: 'left' }}>{category.name}</span>
                <span style={progressPillStyle(doneTopics === category.topics.length)}>
                  {doneTopics}/{category.topics.length} тем
                </span>
                <span style={chevronStyle(isOpen)}>▾</span>
              </button>

              {isOpen && (
                <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', padding: '16px 4px 4px' }}>
                  {category.topics.map((t) => (
                    <button
                      key={t.topicId}
                      onClick={() => setOpenTopicId(t.topicId === openTopicId ? null : t.topicId)}
                      style={topicCardStyle(t.topicId === openTopicId)}
                    >
                      {t.topicName}
                    </button>
                  ))}
              ...
              {isOpen && openTopicId && category.topics.some((t) => t.topicId === openTopicId) && (
...
function categoryHeaderStyle(open: boolean): React.CSSProperties {
  return {
    display: 'flex',
    alignItems: 'center',
    gap: 14,
    ...
    background: open ? COLOR.amberLight : COLOR.surface,
    color: COLOR.text,
    cursor: 'pointer',
    fontFamily: FONT.ui,
    fontSize: 17,
