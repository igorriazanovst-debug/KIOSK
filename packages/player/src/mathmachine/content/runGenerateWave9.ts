// Разовый CLI-скрипт: применяет offline-генератор Этапа 2b, волна 9
// (укрупнённая), к pilotContent.json. Не часть рантайма плеера. Запуск
// (из любой директории):
// node --experimental-strip-types packages/player/src/mathmachine/content/runGenerateWave9.ts

import fs from 'node:fs';
import { fileURLToPath } from 'node:url';
import path from 'node:path';
import type { MathMachineContent } from '@kiosk/shared';
import { mergeIntoContent, countWave9Tasks } from './generateWave9Content.ts';
...
console.log(`Wave 9: added ${countWave9Tasks()} tasks across 14 topics to ${contentPath}`);
