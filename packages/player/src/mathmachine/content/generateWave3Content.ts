...
// runGenerateWave3.ts. Хелперы buildGroup/rotate/DIVISION_DECOY_SCHEMES/
...
function divideTask(a: number, b: number): Omit<Task, 'id'> {
  ...
  const schemeIdx = contentHash('w3-div-scheme-0', [a, b]) % DIVISION_DECOY_SCHEMES.length;
  ...
  const posShift = contentHash('w3-div-position-0', [a, b]);
