...
import { WAVE10_TOPICS, countWave10Tasks, mergeIntoContent } from './generateWave10Content.ts';

test('countWave10Tasks returns the exact expected total (30+30+60+24+20+24+10+24+30+24+24+24+24+30)', () => {
  assert.equal(countWave10Tasks(), 378);
});

test('WAVE10_TOPICS has exactly 14 topics (the fourth enlarged wave)', () => {
  assert.equal(WAVE10_TOPICS.length, 14);
});

test('no duplicate topic, group, or task ids across all of WAVE10_TOPICS', () => {
  const topicIds = new Set<string>();
  const groupIds = new Set<string>();
  const taskIds = new Set<string>();
  for (const topic of WAVE10_TOPICS) {
    ...
    for (const group of topic.groups) {
      for (const task of group.tasks) {
        if (task.typeId === 'number_subtract_two') {
          ...
          assert.ok(a >= 201 && a <= 300, `this wave's subtraction deepening targets minuend 201-300: ${task.id}`);
        }
        if (task.typeId === 'number_sum_two') {
          const { a, b } = task.params as { a: number; b: number };
          assert.equal(task.correctAnswer, a + b);
          assert.ok(a + b >= 200 && a + b <= 300, `this wave's addition deepening targets sums ~200-300: ${task.id}`);
        }
        if (task.typeId === 'number_composition') {
          ...
          assert.ok(whole >= 101 && whole <= 130, `this wave's composition deepening targets 101-130: ${task.id}`);
        ...
        if (task.typeId === 'number_compare') {
          ...
          assert.ok(a >= 700 && b >= 700 && a <= 1000 && b <= 1000, `this wave's comparison deepening targets 700-1000: ${task.id}`);
        ...
        if (task.typeId === 'number_ordering') {
          ...
          assert.equal(task.correctAnswer, expected);
          assert.equal(series.length, 8, `this wave's ordering deepening targets 8-number series: ${task.id}`);
        ...
        if (task.typeId === 'ordinal_position') {
          ...
          assert.ok(series.length === 17 || series.length === 18, `this wave's ordinal deepening targets series length 17 or 18: ${task.id}`);
        ...
        if (task.typeId === 'number_multiply_two') {
          const { a, b } = task.params as { a: number; b: number };
          assert.equal(task.correctAnswer, a * b);
          assert.equal(b, 15, `this wave's multiplication deepening targets ×15: ${task.id}`);
          assert.ok(a >= 1 && a <= 10);
        }
        if (task.typeId === 'number_divide_remainder') {
          const { a, b } = task.params as { a: number; b: number };
          assert.ok(b >= 26 && b <= 30, `this wave's Деление deepening targets divisors 26-30: ${task.id}`);
        ...
        if (task.typeId === 'number_multiple_check') {
          ...
          assert.ok(n >= 31 && n <= 35, `this wave's Кратные deepening targets n=31-35: ${task.id}`);
        ...
        if (task.typeId === 'share_of_whole') {
          ...
          assert.ok(parts === 11 || parts === 12, `this wave's share deepening targets elevenths/twelfths: ${task.id}`);
        ...
        if (task.typeId === 'number_missing') {
          ...
          assert.ok(step === 10 || step === 11, `this wave targets step 10 or 11: ${task.id}`);
        ...
        if (task.typeId === 'number_counting') {
          ...
          assert.ok(count >= 81 && count <= 100, `this wave's counting deepening targets 81-100: ${task.id}`);
        }
        if (task.typeId === 'number_sum_three') {
          ...
          assert.ok(a + b + c >= 75 && a + b + c <= 110, `this wave's sum-of-three deepening targets sums ~75-110: ${task.id}`);
        }
...
test('WAVE10_TOPICS reuses no round_to_ten (thousands) number already used by waves 8 or 9', () => {
  ...
  for (const topic of WAVE10_TOPICS) {
    for (const group of topic.groups) {
      for (const task of group.tasks) {
        if (task.typeId === 'round_to_ten') {
          const { n } = task.params as { n: number };
          assert.ok(!PRIOR_THOUSAND_NUMBERS.has(n), `number ${n} was already used by wave 8 or 9's rounding topic: ${task.id}`);
        }
      }
    }
  }
...
test('within "Порядок чисел: восемь чисел", the minimum/maximum does not always land on the same series position', () => {
  let checkedGroups = 0;
  for (const topic of WAVE10_TOPICS) {
    for (const group of topic.groups) {
      const tasks = group.tasks.filter((t) => t.typeId === 'number_ordering' && (t.params as { series: number[] }).series.length === 8);
...
test('mergeIntoContent adds all wave 10 topics without touching existing content', () => {
  const merged = mergeIntoContent(BASE_CONTENT);
  assert.ok(merged.topics.top_addition, 'existing topic must survive the merge');
  assert.equal(Object.keys(merged.topics).length, 1 + WAVE10_TOPICS.length);
  const arithmeticSection = merged.sections.find((s) => s.id === 'sec_arithmetic')!;
  assert.equal(arithmeticSection.topicIds.length, 1 + WAVE10_TOPICS.length);
  assert.equal(Object.keys(merged.tasks).length, 1 + countWave10Tasks());
});

test('mergeIntoContent throws instead of silently overwriting a colliding topic id', () => {
  const colliding: MathMachineContent = {
    ...BASE_CONTENT,
    topics: { ...BASE_CONTENT.topics, top_composition_7: { id: 'top_composition_7', name: 'existing', groupIds: ['x'] } },
  };
  assert.throws(() => mergeIntoContent(colliding), /top_composition_7/);
});
