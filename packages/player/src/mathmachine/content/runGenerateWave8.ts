// Разовый CLI-скрипт: применяет offline-генератор Этапа 2b, волна 8
// (укрупнённая), к pilotContent.json. Не часть рантайма плеера. Запуск
// (из любой директории):
// node --experimental-strip-types packages/player/src/mathmachine/content/runGenerateWave8.ts

import fs from 'node:fs';
import { fileURLToPath } from 'node:url';
import path from 'node:path';
import type { MathMachineContent } from '@kiosk/shared';
import { mergeIntoContent, countWave8Tasks } from './generateWave8Content.ts';
...
console.log(`Wave 8: added ${countWave8Tasks()} tasks across 14 topics to ${contentPath}`);
