...
import { WAVE9_TOPICS, countWave9Tasks, mergeIntoContent } from './generateWave9Content.ts';

test('countWave9Tasks returns the exact expected total (30+30+60+24+20+24+20+24+30+24+24+24+24+30)', () => {
  assert.equal(countWave9Tasks(), 388);
});

test('WAVE9_TOPICS has exactly 14 topics (the third enlarged wave)', () => {
  assert.equal(WAVE9_TOPICS.length, 14);
});

test('no duplicate topic, group, or task ids across all of WAVE9_TOPICS', () => {
  const topicIds = new Set<string>();
  const groupIds = new Set<string>();
  const taskIds = new Set<string>();
  for (const topic of WAVE9_TOPICS) {
    ...
    for (const group of topic.groups) {
      for (const task of group.tasks) {
        if (task.typeId === 'number_subtract_two') {
          ...
          assert.ok(a >= 101 && a <= 200, `this wave's subtraction deepening targets minuend 101-200: ${task.id}`);
        }
        if (task.typeId === 'number_sum_two') {
          const { a, b } = task.params as { a: number; b: number };
          assert.equal(task.correctAnswer, a + b);
          assert.ok(a + b >= 100 && a + b <= 200, `this wave's addition deepening targets sums ~100-200: ${task.id}`);
        }
        if (task.typeId === 'number_composition') {
          ...
          assert.ok(whole >= 71 && whole <= 100, `this wave's composition deepening targets 71-100: ${task.id}`);
        ...
        if (task.typeId === 'number_compare') {
          ...
          assert.ok(a >= 200 && b >= 200 && a <= 500 && b <= 500, `this wave's comparison deepening targets 200-500: ${task.id}`);
        ...
        if (task.typeId === 'number_ordering') {
          ...
          assert.equal(task.correctAnswer, expected);
          assert.equal(series.length, 7, `this wave's ordering deepening targets 7-number series: ${task.id}`);
        ...
        if (task.typeId === 'ordinal_position') {
          ...
          assert.ok(series.length === 15 || series.length === 16, `this wave's ordinal deepening targets series length 15 or 16: ${task.id}`);
        ...
        if (task.typeId === 'number_multiply_two') {
          const { a, b } = task.params as { a: number; b: number };
          assert.equal(task.correctAnswer, a * b);
          assert.ok(b === 13 || b === 14, `this wave's multiplication deepening targets ×13/×14: ${task.id}`);
          assert.ok(a >= 1 && a <= 10);
        }
        if (task.typeId === 'number_divide_remainder') {
          const { a, b } = task.params as { a: number; b: number };
          assert.ok(b >= 21 && b <= 25, `this wave's Деление deepening targets divisors 21-25: ${task.id}`);
        ...
        if (task.typeId === 'number_multiple_check') {
          ...
          assert.ok(n >= 26 && n <= 30, `this wave's Кратные deepening targets n=26-30: ${task.id}`);
        ...
        if (task.typeId === 'share_of_whole') {
          ...
          assert.ok(parts === 7 || parts === 9, `this wave's share deepening targets sevenths/ninths: ${task.id}`);
        ...
        if (task.typeId === 'number_missing') {
          ...
          assert.ok(step === 8 || step === 9, `this wave targets step 8 or 9: ${task.id}`);
        ...
        if (task.typeId === 'number_counting') {
          ...
          assert.ok(count >= 61 && count <= 80, `this wave's counting deepening targets 61-80: ${task.id}`);
        }
        if (task.typeId === 'number_sum_three') {
          ...
          assert.ok(a + b + c >= 55 && a + b + c <= 85, `this wave's sum-of-three deepening targets sums ~55-85: ${task.id}`);
        }
...
test('WAVE9_TOPICS reuses no round_to_ten (thousands) number already used by wave 8', () => {
  const WAVE8_THOUSAND_NUMBERS = new Set([
  ...
  for (const topic of WAVE9_TOPICS) {
    for (const group of topic.groups) {
      for (const task of group.tasks) {
        if (task.typeId === 'round_to_ten') {
          const { n } = task.params as { n: number };
          assert.ok(!WAVE8_THOUSAND_NUMBERS.has(n), `number ${n} was already used by wave 8's rounding topic: ${task.id}`);
        }
      }
    }
  }
...
test('within "Порядок чисел: семь чисел", the minimum/maximum does not always land on the same series position', () => {
  let checkedGroups = 0;
  for (const topic of WAVE9_TOPICS) {
    for (const group of topic.groups) {
      const tasks = group.tasks.filter((t) => t.typeId === 'number_ordering' && (t.params as { series: number[] }).series.length === 7);
...
test('mergeIntoContent adds all wave 9 topics without touching existing content', () => {
  const merged = mergeIntoContent(BASE_CONTENT);
  assert.ok(merged.topics.top_addition, 'existing topic must survive the merge');
  assert.equal(Object.keys(merged.topics).length, 1 + WAVE9_TOPICS.length);
  const arithmeticSection = merged.sections.find((s) => s.id === 'sec_arithmetic')!;
  assert.equal(arithmeticSection.topicIds.length, 1 + WAVE9_TOPICS.length);
  assert.equal(Object.keys(merged.tasks).length, 1 + countWave9Tasks());
});

test('mergeIntoContent throws instead of silently overwriting a colliding topic id', () => {
  const colliding: MathMachineContent = {
    ...BASE_CONTENT,
    topics: { ...BASE_CONTENT.topics, top_composition_6: { id: 'top_composition_6', name: 'existing', groupIds: ['x'] } },
  };
  assert.throws(() => mergeIntoContent(colliding), /top_composition_6/);
});
