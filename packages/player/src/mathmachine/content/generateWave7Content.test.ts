...
import { WAVE7_TOPICS, countWave7Tasks, mergeIntoContent } from './generateWave7Content.ts';

test('countWave7Tasks returns the exact expected total (30+30+40+24+20+24+30+24+24+24+24+24+30)', () => {
  assert.equal(countWave7Tasks(), 348);
});

test('WAVE7_TOPICS has exactly 13 topics (the enlarged wave)', () => {
  assert.equal(WAVE7_TOPICS.length, 13);
});

test('no duplicate topic, group, or task ids across all of WAVE7_TOPICS', () => {
  const topicIds = new Set<string>();
  const groupIds = new Set<string>();
  const taskIds = new Set<string>();
  for (const topic of WAVE7_TOPICS) {
    ...
    for (const group of topic.groups) {
      for (const task of group.tasks) {
        if (task.typeId === 'number_subtract_two') {
          ...
          assert.ok(a >= 21 && a <= 50, `this wave's subtraction deepening targets minuend 21-50: ${task.id}`);
        }
        if (task.typeId === 'number_sum_two') {
          const { a, b } = task.params as { a: number; b: number };
          assert.equal(task.correctAnswer, a + b);
          assert.ok(a + b >= 20 && a + b <= 55, `this wave's addition deepening targets sums ~20-55: ${task.id}`);
        }
        if (task.typeId === 'number_composition') {
          ...
          assert.ok(whole >= 31 && whole <= 50, `this wave's composition deepening targets 31-50: ${task.id}`);
        ...
        if (task.typeId === 'number_compare') {
          ...
          assert.ok(a >= 50 && b >= 50 && a <= 99 && b <= 99, `this wave's comparison deepening targets 50-99: ${task.id}`);
        ...
        if (task.typeId === 'number_ordering') {
          ...
          assert.equal(task.correctAnswer, expected);
          assert.equal(series.length, 5, `this wave's ordering deepening targets 5-number series: ${task.id}`);
        ...
        if (task.typeId === 'ordinal_position') {
          ...
          assert.ok(series.length === 11 || series.length === 12, `this wave's ordinal deepening targets series length 11 or 12: ${task.id}`);
        ...
        if (task.typeId === 'number_multiple_check') {
          ...
          assert.ok(n >= 16 && n <= 20, `this wave's Кратные deepening targets n=16-20: ${task.id}`);
        ...
        if (task.typeId === 'number_divide_remainder') {
          const { a, b } = task.params as { a: number; b: number };
          assert.ok(b === 13 || b === 14 || b === 15, `this wave's Деление deepening targets divisors 13-15: ${task.id}`);
        ...
        if (task.typeId === 'share_of_whole') {
          ...
          assert.ok(parts === 5 || parts === 10, `this wave's share deepening targets fifths/tenths: ${task.id}`);
        ...
        if (task.typeId === 'number_missing') {
          ...
          assert.ok(step === 4 || step === 5, `this wave targets step 4 or 5: ${task.id}`);
        ...
        if (task.typeId === 'number_counting') {
          ...
          assert.ok(count >= 21 && count <= 40, `this wave's counting deepening targets 21-40: ${task.id}`);
        }
        if (task.typeId === 'number_sum_three') {
          ...
          assert.ok(a + b + c >= 21 && a + b + c <= 40, `this wave's sum-of-three deepening targets sums 21-40: ${task.id}`);
        }
...
test('WAVE7_TOPICS reuses no round_to_ten number already used by wave 4', () => {
  const WAVE4_ROUND_NUMBERS = new Set([120, 170, 230, 280, 340, 410, 470, 530, 580, 620, 690, 740, 810, 860, 930, 970]);
  for (const topic of WAVE7_TOPICS) {
    for (const group of topic.groups) {
      for (const task of group.tasks) {
        if (task.typeId === 'round_to_ten') {
          const { n } = task.params as { n: number };
          assert.ok(!WAVE4_ROUND_NUMBERS.has(n), `number ${n} was already used by wave 4's rounding topic: ${task.id}`);
        }
      }
    }
  }
...
test('within every choice-mode group (Сравнение, Порядок чисел, Кратные, Деление), the button position is not exploitable (frequency/period/linear formula)', () => {
  ...
  assert.equal(checkedGroups, 10, 'expected exactly 10 choice-mode groups (3 Сравнение + 2 Порядок чисел + 2 Кратные + 3 Деление)');
});

test('within "Порядок чисел: пять чисел", the minimum/maximum does not always land on the same series position', () => {
  let checkedGroups = 0;
  for (const topic of WAVE7_TOPICS) {
    for (const group of topic.groups) {
      const tasks = group.tasks.filter((t) => t.typeId === 'number_ordering' && (t.params as { series: number[] }).series.length === 5);
...
test('within "Сравнение" (this wave), the number-naming order in the question text is not exploitable', () => {
  ...
  for (const topic of WAVE7_TOPICS) {
    for (const group of topic.groups) {
      ...
      for (const t of tasks) {
        const { a } = t.params as { a: number; b: number };
...
test('mergeIntoContent adds all wave 7 topics without touching existing content', () => {
  const merged = mergeIntoContent(BASE_CONTENT);
  assert.ok(merged.topics.top_addition, 'existing topic must survive the merge');
  assert.equal(Object.keys(merged.topics).length, 1 + WAVE7_TOPICS.length);
  const arithmeticSection = merged.sections.find((s) => s.id === 'sec_arithmetic')!;
  assert.equal(arithmeticSection.topicIds.length, 1 + WAVE7_TOPICS.length);
  assert.equal(Object.keys(merged.tasks).length, 1 + countWave7Tasks());
});

test('mergeIntoContent throws instead of silently overwriting a colliding topic id', () => {
  const colliding: MathMachineContent = {
    ...BASE_CONTENT,
    topics: { ...BASE_CONTENT.topics, top_composition_4: { id: 'top_composition_4', name: 'existing', groupIds: ['x'] } },
  };
  assert.throws(() => mergeIntoContent(colliding), /top_composition_4/);
});
