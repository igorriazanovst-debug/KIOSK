...
import { WAVE11_TOPICS, countWave11Tasks, mergeIntoContent } from './generateWave11Content.ts';

test('countWave11Tasks returns the exact expected total (30+30+60+24+20+24+30+24+30+24+24+24+30)', () => {
  assert.equal(countWave11Tasks(), 374);
});

test('WAVE11_TOPICS has exactly 13 topics (the fifth enlarged wave)', () => {
  assert.equal(WAVE11_TOPICS.length, 13);
});

test('no duplicate topic, group, or task ids across all of WAVE11_TOPICS', () => {
  const topicIds = new Set<string>();
  const groupIds = new Set<string>();
  const taskIds = new Set<string>();
  for (const topic of WAVE11_TOPICS) {
...
test('WAVE11_TOPICS does not include a "Доли целого" topic (school denominator set 2-12 completed in wave 10)', () => {
  for (const topic of WAVE11_TOPICS) {
    for (const group of topic.groups) {
      for (const task of group.tasks) {
        assert.notEqual(task.typeId, 'share_of_whole', `wave 11 should not add share_of_whole tasks: ${task.id}`);
      }
    }
  }
});
    ...
    for (const group of topic.groups) {
      for (const task of group.tasks) {
        if (task.typeId === 'number_subtract_two') {
          ...
          assert.ok(a >= 301 && a <= 500, `this wave's subtraction deepening targets minuend 301-500: ${task.id}`);
        }
        if (task.typeId === 'number_sum_two') {
          const { a, b } = task.params as { a: number; b: number };
          assert.equal(task.correctAnswer, a + b);
          assert.ok(a + b >= 300 && a + b <= 500, `this wave's addition deepening targets sums ~300-500: ${task.id}`);
        }
        if (task.typeId === 'number_composition') {
          ...
          assert.ok(whole >= 131 && whole <= 160, `this wave's composition deepening targets 131-160: ${task.id}`);
        ...
        if (task.typeId === 'number_compare') {
          ...
          assert.ok(a >= 1500 && b >= 1500 && a <= 2000 && b <= 2000, `this wave's comparison deepening targets 1500-2000: ${task.id}`);
        ...
        if (task.typeId === 'number_ordering') {
          ...
          assert.equal(task.correctAnswer, expected);
          assert.equal(series.length, 9, `this wave's ordering deepening targets 9-number series: ${task.id}`);
        ...
        if (task.typeId === 'ordinal_position') {
          ...
          assert.ok(series.length === 19 || series.length === 20, `this wave's ordinal deepening targets series length 19 or 20: ${task.id}`);
        ...
        if (task.typeId === 'number_multiply_two') {
          const { a, b } = task.params as { a: number; b: number };
          assert.equal(task.correctAnswer, a * b);
          assert.ok(a >= 11 && a <= 20, `this wave's multiplication deepening targets a two-digit multiplicand 11-20: ${task.id}`);
        ...
        if (task.typeId === 'number_divide_remainder') {
          const { a, b } = task.params as { a: number; b: number };
          assert.ok(b >= 31 && b <= 35, `this wave's Деление deepening targets divisors 31-35: ${task.id}`);
        ...
        if (task.typeId === 'number_multiple_check') {
          ...
          assert.ok(n >= 36 && n <= 40, `this wave's Кратные deepening targets n=36-40: ${task.id}`);
        ...
        if (task.typeId === 'number_missing') {
          ...
          assert.ok(step === 12 || step === 13, `this wave targets step 12 or 13: ${task.id}`);
        ...
        if (task.typeId === 'number_counting') {
          ...
          assert.ok(count >= 101 && count <= 120, `this wave's counting deepening targets 101-120: ${task.id}`);
        }
        if (task.typeId === 'number_sum_three') {
          ...
          assert.ok(a + b + c >= 100 && a + b + c <= 135, `this wave's sum-of-three deepening targets sums ~100-135: ${task.id}`);
        }
...
test('WAVE11_TOPICS reuses no round_to_ten (thousands) number already used by waves 8, 9, or 10', () => {
...
test('within "Порядок чисел: девять чисел", the minimum/maximum does not always land on the same series position', () => {
  let checkedGroups = 0;
  for (const topic of WAVE11_TOPICS) {
    for (const group of topic.groups) {
      const tasks = group.tasks.filter((t) => t.typeId === 'number_ordering' && (t.params as { series: number[] }).series.length === 9);
...
test('mergeIntoContent adds all wave 11 topics without touching existing content', () => {
  const merged = mergeIntoContent(BASE_CONTENT);
  assert.ok(merged.topics.top_addition, 'existing topic must survive the merge');
  assert.equal(Object.keys(merged.topics).length, 1 + WAVE11_TOPICS.length);
  const arithmeticSection = merged.sections.find((s) => s.id === 'sec_arithmetic')!;
  assert.equal(arithmeticSection.topicIds.length, 1 + WAVE11_TOPICS.length);
  assert.equal(Object.keys(merged.tasks).length, 1 + countWave11Tasks());
});

test('mergeIntoContent throws instead of silently overwriting a colliding topic id', () => {
  const colliding: MathMachineContent = {
    ...BASE_CONTENT,
    topics: { ...BASE_CONTENT.topics, top_composition_8: { id: 'top_composition_8', name: 'existing', groupIds: ['x'] } },
  };
  assert.throws(() => mergeIntoContent(colliding), /top_composition_8/);
});
