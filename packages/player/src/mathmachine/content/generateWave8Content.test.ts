...
import { WAVE8_TOPICS, countWave8Tasks, mergeIntoContent } from './generateWave8Content.ts';

test('countWave8Tasks returns the exact expected total (30+30+40+24+20+24+20+24+30+24+24+24+24+30)', () => {
  assert.equal(countWave8Tasks(), 368);
});

test('WAVE8_TOPICS has exactly 14 topics (the second enlarged wave)', () => {
  assert.equal(WAVE8_TOPICS.length, 14);
});

test('no duplicate topic, group, or task ids across all of WAVE8_TOPICS', () => {
  const topicIds = new Set<string>();
  const groupIds = new Set<string>();
  const taskIds = new Set<string>();
  for (const topic of WAVE8_TOPICS) {
    ...
    for (const group of topic.groups) {
      for (const task of group.tasks) {
        if (task.typeId === 'number_subtract_two') {
          ...
          assert.ok(a >= 51 && a <= 100, `this wave's subtraction deepening targets minuend 51-100: ${task.id}`);
        }
        if (task.typeId === 'number_sum_two') {
          const { a, b } = task.params as { a: number; b: number };
          assert.equal(task.correctAnswer, a + b);
          assert.ok(a + b >= 50 && a + b <= 100, `this wave's addition deepening targets sums ~50-100: ${task.id}`);
        }
        if (task.typeId === 'number_composition') {
          ...
          assert.ok(whole >= 51 && whole <= 70, `this wave's composition deepening targets 51-70: ${task.id}`);
        ...
        if (task.typeId === 'number_compare') {
          ...
          assert.ok(a >= 100 && b >= 100 && a <= 199 && b <= 199, `this wave's comparison deepening targets 100-199: ${task.id}`);
        ...
        if (task.typeId === 'number_ordering') {
          ...
          assert.equal(task.correctAnswer, expected);
          assert.equal(series.length, 6, `this wave's ordering deepening targets 6-number series: ${task.id}`);
        ...
        if (task.typeId === 'ordinal_position') {
          ...
          assert.ok(series.length === 13 || series.length === 14, `this wave's ordinal deepening targets series length 13 or 14: ${task.id}`);
        ...
        if (task.typeId === 'number_multiply_two') {
          const { a, b } = task.params as { a: number; b: number };
          assert.equal(task.correctAnswer, a * b);
          assert.ok(b === 11 || b === 12, `this wave's multiplication deepening targets ×11/×12: ${task.id}`);
          assert.ok(a >= 1 && a <= 10);
        }
        if (task.typeId === 'number_divide_remainder') {
          const { a, b } = task.params as { a: number; b: number };
          assert.ok(b >= 16 && b <= 20, `this wave's Деление deepening targets divisors 16-20: ${task.id}`);
        ...
        if (task.typeId === 'number_multiple_check') {
          ...
          assert.ok(n >= 21 && n <= 25, `this wave's Кратные deepening targets n=21-25: ${task.id}`);
        ...
        if (task.typeId === 'share_of_whole') {
          ...
          assert.ok(parts === 6 || parts === 8, `this wave's share deepening targets sixths/eighths: ${task.id}`);
        ...
        if (task.typeId === 'number_missing') {
          ...
          assert.ok(step === 6 || step === 7, `this wave targets step 6 or 7: ${task.id}`);
        ...
        if (task.typeId === 'number_counting') {
          ...
          assert.ok(count >= 41 && count <= 60, `this wave's counting deepening targets 41-60: ${task.id}`);
        }
        if (task.typeId === 'number_sum_three') {
          ...
          assert.ok(a + b + c >= 40 && a + b + c <= 65, `this wave's sum-of-three deepening targets sums ~40-65: ${task.id}`);
        }
...
test('within "Порядок чисел: шесть чисел", the minimum/maximum does not always land on the same series position', () => {
  let checkedGroups = 0;
  for (const topic of WAVE8_TOPICS) {
    for (const group of topic.groups) {
      const tasks = group.tasks.filter((t) => t.typeId === 'number_ordering' && (t.params as { series: number[] }).series.length === 6);
...
test('WAVE8_TOPICS round_to_ten (thousands) never repeats a wave 4/7 hundreds number and never lands on an exact half', () => {
  for (const topic of WAVE8_TOPICS) {
    for (const group of topic.groups) {
      for (const task of group.tasks) {
        if (task.typeId === 'round_to_ten') {
          const { n } = task.params as { n: number };
          assert.ok(n % 1000 !== 500, `number ${n} lands exactly on the thousands half: ${task.id}`);
        }
      }
    }
  }
...
test('mergeIntoContent adds all wave 8 topics without touching existing content', () => {
  const merged = mergeIntoContent(BASE_CONTENT);
  assert.ok(merged.topics.top_addition, 'existing topic must survive the merge');
  assert.equal(Object.keys(merged.topics).length, 1 + WAVE8_TOPICS.length);
  const arithmeticSection = merged.sections.find((s) => s.id === 'sec_arithmetic')!;
  assert.equal(arithmeticSection.topicIds.length, 1 + WAVE8_TOPICS.length);
  assert.equal(Object.keys(merged.tasks).length, 1 + countWave8Tasks());
});

test('mergeIntoContent throws instead of silently overwriting a colliding topic id', () => {
  const colliding: MathMachineContent = {
    ...BASE_CONTENT,
    topics: { ...BASE_CONTENT.topics, top_composition_5: { id: 'top_composition_5', name: 'existing', groupIds: ['x'] } },
  };
  assert.throws(() => mergeIntoContent(colliding), /top_composition_5/);
});
