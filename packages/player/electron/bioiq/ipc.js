// packages/player/electron/bioiq/ipc.js
// Единственная точка, через которую рендерер (виджет «БиоIQ») получает
...
const BIOIQ_APP_DIR_NAME = 'kiosk-bioiq';
...
// сверке с ТЗ, docs/bioiq-acceptance-matrix.md §3).
class BioiqStoreError extends Error {
  constructor(message) {
    super(message);
    this.name = 'BioiqStoreError';
  }
}

function resolveBaseDir(app) {
  try {
    const dir = path.join(app.getPath('appData'), BIOIQ_APP_DIR_NAME);
    fs.mkdirSync(dir, { recursive: true });
    return { baseDir: dir, isFallback: false };
  } catch (err) {
    const dir = path.join(require('os').tmpdir(), BIOIQ_APP_DIR_NAME);
    fs.mkdirSync(dir, { recursive: true });
...
function readUserData(filePath) {
  ...
  if (!status.valid || !isPlainRecord(status.data)) {
    throw new BioiqStoreError(`Файл данных БиоIQ повреждён: ${path.basename(filePath)}`);
  }
  return status.data;
}
 ...
 */
function registerBioiqIpc({ ipcMain, app }) {
  ...
  ipcMain.handle('bioiq:load-user-data', () => readUserData(filePath));
  ipcMain.handle('bioiq:save-user-data', (_event, data) => {
    if (!isPlainRecord(data)) return { ok: false };
    writeUserDataAtomic(filePath, data);
    return { ok: true };
  });

  ipcMain.handle('bioiq:list-quizzes', () => listQuizMetadata(quizzesDir));
  ipcMain.handle('bioiq:load-quiz', (_event, quizId) => {
    if (typeof quizId !== 'string' || quizId.length === 0) return null;
    return loadQuizFile(quizzesDir, quizId);
  });
  ipcMain.handle('bioiq:save-quiz', (_event, quiz) => {
  ...
  ipcMain.handle('bioiq:delete-quiz', (_event, quizId) => {
    if (typeof quizId !== 'string' || quizId.length === 0) return { ok: false };
    return { ok: deleteQuizFile(quizzesDir, quizId) };
  });

  ipcMain.handle('bioiq:save-quiz-level-image', (_event, quizId, level, arrayBuffer, mimeType) => {
    if (typeof quizId !== 'string' || quizId.length === 0) return { ok: false };
    try {
  ...
  ipcMain.handle('bioiq:save-quiz-item-image', (_event, quizId, questionId, kind, arrayBuffer, mimeType) => {
    if (typeof quizId !== 'string' || quizId.length === 0) return { ok: false };
    try {
  ...
  ipcMain.handle('bioiq:delete-quiz-item-image', (_event, fileName) => {
