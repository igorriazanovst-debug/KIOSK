// packages/player/electron/bioiq/ipc.test.js
import { test } from 'node:test';
import assert from 'node:assert/strict';
import fs from 'node:fs';
import os from 'node:os';
import path from 'node:path';
import { registerBioiqIpc, readUserData, writeUserDataAtomic, resolveBaseDir, listQuizMetadata, loadQuizFile, saveQuizFile, deleteQuizFile, resolveQuizzesDir, saveQuizLevelImage, saveQuizItemImage, deleteQuizItemImage, BioiqStoreError } from './ipc.js';
...
// результат/ошибку рендереру через Promise (см. registerBioiqIpc load
...
test('resolveBaseDir creates and returns a directory under app.getPath("appData")', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-ipc-appdata-'));
  ...
  assert.equal(baseDir, path.join(tmp, 'kiosk-bioiq'));
});

test('readUserData returns the fallback when the file does not exist', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-ipc-read-'));
...
test('writeUserDataAtomic then readUserData round-trips the same data', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-ipc-roundtrip-'));
...
test('readUserData throws BioiqStoreError when the file exists but is not valid JSON', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-ipc-corrupt-'));
  const filePath = path.join(tmp, 'userdata.json');
  fs.writeFileSync(filePath, '{not valid json');
  assert.throws(() => readUserData(filePath), BioiqStoreError);
});

test('readUserData throws BioiqStoreError when the file is valid JSON but not a record (e.g. an array)', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-ipc-corrupt-shape-'));
  const filePath = path.join(tmp, 'userdata.json');
  fs.writeFileSync(filePath, '[1,2,3]');
  assert.throws(() => readUserData(filePath), BioiqStoreError);
});

test('registerBioiqIpc load handler rejects when userdata.json is corrupted, without touching the file', async () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-ipc-register-corrupt-'));
  const app = { getPath: () => tmp };
  const ipcMain = fakeIpcMain();
  registerBioiqIpc({ ipcMain, app });
  const filePath = path.join(tmp, 'kiosk-bioiq', 'userdata.json');
  fs.writeFileSync(filePath, '{not valid json');

  await assert.rejects(() => ipcMain.invoke('bioiq:load-user-data'), BioiqStoreError);
  // Повреждённый файл не должен быть молча перезаписан фолбэком.
  assert.equal(fs.readFileSync(filePath, 'utf-8'), '{not valid json');
});

test('registerBioiqIpc wires load/save handlers end-to-end', async () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-ipc-register-'));
  const app = { getPath: () => tmp };
  const ipcMain = fakeIpcMain();
  registerBioiqIpc({ ipcMain, app });

  const initial = await ipcMain.invoke('bioiq:load-user-data');
  assert.deepEqual(initial, { schemaVersion: 1, sessions: [], soundOn: true });

  const saveResult = await ipcMain.invoke('bioiq:save-user-data', { schemaVersion: 1, sessions: [], soundOn: false });
  assert.deepEqual(saveResult, { ok: true });

  const reloaded = await ipcMain.invoke('bioiq:load-user-data');
  assert.equal(reloaded.soundOn, false);
});

test('registerBioiqIpc save handler rejects non-object payloads', async () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-ipc-reject-'));
  const app = { getPath: () => tmp };
  const ipcMain = fakeIpcMain();
  registerBioiqIpc({ ipcMain, app });
  const result = await ipcMain.invoke('bioiq:save-user-data', 'not-an-object');
  assert.deepEqual(result, { ok: false });
});

test('resolveQuizzesDir creates and returns a "quizzes" subdirectory', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-quizzes-dir-'));
...
test('listQuizMetadata returns an empty array when the directory has no quiz files', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-quizzes-empty-'));
  const dir = resolveQuizzesDir(tmp);
  assert.deepEqual(listQuizMetadata(dir), []);
});

test('saveQuizFile then loadQuizFile round-trips the same quiz object', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-quizzes-roundtrip-'));
  const dir = resolveQuizzesDir(tmp);
...
test('loadQuizFile returns null for a missing quiz id', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-quizzes-missing-'));
  const dir = resolveQuizzesDir(tmp);
...
test('listQuizMetadata reflects saved quizzes with title, hasPassword and updatedAt', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-quizzes-list-'));
  const dir = resolveQuizzesDir(tmp);
...
test('listQuizMetadata skips a corrupted quiz file instead of throwing', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-quizzes-corrupt-'));
  const dir = resolveQuizzesDir(tmp);
...
test('deleteQuizFile removes an existing file and returns true', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-quizzes-delete-'));
  const dir = resolveQuizzesDir(tmp);
...
test('deleteQuizFile returns false for a missing quiz id', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-quizzes-delete-missing-'));
  const dir = resolveQuizzesDir(tmp);
...
test('deleteQuizFile removes all per-level images named in quiz.images', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-quizzes-delete-images-'));
  const dir = resolveQuizzesDir(tmp);
...
test('deleteQuizFile still removes the quiz JSON when a referenced level image is already missing from disk', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-quizzes-delete-img-missing-'));
  const dir = resolveQuizzesDir(tmp);
  saveQuizFile(dir, {
    id: 'quiz-orphan-ref',
...
test('deleteQuizFile removes only the JSON when the quiz has no images', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-quizzes-delete-no-img-'));
  const dir = resolveQuizzesDir(tmp);
...
test('registerBioiqIpc wires quiz catalog handlers end-to-end', async () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-quizzes-e2e-'));
  const app = { getPath: () => tmp };
  const ipcMain = fakeIpcMain();
  registerBioiqIpc({ ipcMain, app });

  assert.deepEqual(await ipcMain.invoke('bioiq:list-quizzes'), []);

  const saveResult = await ipcMain.invoke('bioiq:save-quiz', { id: 'quiz-e2e', title: 'E2E', passwordHash: null, questions: [] });
  assert.deepEqual(saveResult, { ok: true });

  const loaded = await ipcMain.invoke('bioiq:load-quiz', 'quiz-e2e');
  assert.equal(loaded.title, 'E2E');

  const list = await ipcMain.invoke('bioiq:list-quizzes');
  assert.equal(list.length, 1);
  assert.equal(list[0].id, 'quiz-e2e');

  const deleteResult = await ipcMain.invoke('bioiq:delete-quiz', 'quiz-e2e');
  assert.deepEqual(deleteResult, { ok: true });
  assert.equal(await ipcMain.invoke('bioiq:load-quiz', 'quiz-e2e'), null);
});

test('registerBioiqIpc save-quiz handler rejects a payload without a string id', async () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-quizzes-reject-'));
  const app = { getPath: () => tmp };
  const ipcMain = fakeIpcMain();
  registerBioiqIpc({ ipcMain, app });
  const result = await ipcMain.invoke('bioiq:save-quiz', { title: 'Без id' });
  assert.deepEqual(result, { ok: false });
});

test('saveQuizLevelImage writes a file named "<quizId>-level<N>.<ext>" and returns its fileName', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-lvl-'));
  const dir = resolveQuizzesDir(tmp);
...
test('saveQuizLevelImage rejects an out-of-range level', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-lvl-badlevel-'));
  const dir = resolveQuizzesDir(tmp);
...
test('saveQuizLevelImage rejects an unsupported mime type', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-lvl-reject-'));
  const dir = resolveQuizzesDir(tmp);
  const result = saveQuizLevelImage(dir, 'quiz-lvl-3', 1, Buffer.from([1, 2, 3]), 'application/pdf');
  assert.deepEqual(result, { ok: false });
});

test('registerBioiqIpc save-quiz-level-image handler round-trips through IPC', async () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-lvl-ipc-'));
  const app = { getPath: () => tmp };
  const ipcMain = fakeIpcMain();
  const { quizzesDir } = registerBioiqIpc({ ipcMain, app });
  const buffer = new Uint8Array([0xff, 0xd8, 0xff]).buffer;
  const result = await ipcMain.invoke('bioiq:save-quiz-level-image', 'quiz-lvl-4', 3, buffer, 'image/jpeg');
...
test('saveQuizItemImage writes a file named "<quizId>-<questionId>-<kind>.<ext>" and returns its fileName', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-item-img-'));
  const dir = resolveQuizzesDir(tmp);
  const buffer = Buffer.from([0x89, 0x50, 0x4e, 0x47]);
...
test('saveQuizItemImage rejects an unknown kind', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-item-img-badkind-'));
  const dir = resolveQuizzesDir(tmp);
...
test('saveQuizItemImage rejects an unsupported mime type', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-item-img-badmime-'));
  const dir = resolveQuizzesDir(tmp);
...
test('deleteQuizItemImage removes an existing file and returns true', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-item-img-delete-'));
  const dir = resolveQuizzesDir(tmp);
...
test('deleteQuizItemImage returns false for a missing file without throwing', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-item-img-delete-missing-'));
  const dir = resolveQuizzesDir(tmp);
  assert.equal(deleteQuizItemImage(dir, 'does-not-exist.png'), false);
});

test('registerBioiqIpc save/delete-quiz-item-image handlers round-trip through IPC', async () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-item-img-ipc-'));
  const app = { getPath: () => tmp };
  const ipcMain = fakeIpcMain();
  const { quizzesDir } = registerBioiqIpc({ ipcMain, app });
  const buffer = new Uint8Array([0xff, 0xd8, 0xff]).buffer;
  const saveResult = await ipcMain.invoke('bioiq:save-quiz-item-image', 'quiz-9', 'q-9', 'answer', buffer, 'image/jpeg');
  ...
  const deleteResult = await ipcMain.invoke('bioiq:delete-quiz-item-image', saveResult.fileName);
  assert.deepEqual(deleteResult, { ok: true });
...
test('deleteQuizFile also removes question/answer/hint images referenced by questions', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-quizzes-delete-itemimgs-'));
  const dir = resolveQuizzesDir(tmp);
...
test('loadQuizFile and deleteQuizFile reject a path-traversal quiz id instead of touching files outside quizzesDir', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bioiq-quizzes-traversal-'));
  const dir = resolveQuizzesDir(tmp);
