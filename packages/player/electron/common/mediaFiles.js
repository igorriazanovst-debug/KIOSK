// packages/player/electron/common/mediaFiles.js
// Хранение картинок и записей педагога ФАЙЛАМИ на диске — общее для виджетов.
//
// Вынесено из electron/words/ при работе над Тип 3: механизм понадобился
// второму виджету без единого изменения — здесь нет ничего, что знало бы про
// слова, темы или буквы. План Тип 3 относил этот подъём к Фазе 5; здесь он
// и сделан.
//
// Каталог хранения приходит параметром (baseDir), поэтому у каждого виджета
// он свой: %ProgramData%\kiosk-words\media и %ProgramData%\kiosk-alphabet\media.
// Смешивать их нельзя — удаление виджета не должно уносить чужие файлы.
//
