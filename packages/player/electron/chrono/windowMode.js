...
function buildBrowserWindowOptions(projectData) {
  ...
  return {
    ...
    minimizable: true,
    maximizable: true,
    // Найдено вживую при реальном запуске в Electron: без этого флага
    // width/height (BASE_WINDOW_OPTIONS) задают размер ОКНА С РАМКОЙ, а не
    // содержимого - на Windows resizable-рамка съедает несколько px с
    // каждой стороны, и доска Хронолинии (которая доверяет ЗАЯВЛЕННОЙ
    // ширине виджета как площади для отрисовки, п. "в границах виджета",
    // Фаза 0) рендерилась на эти же несколько px шире реально видимой
    // области - правый край дорожки события (и кнопка "+" на нём)
    // оказывался частично обрезан. useContentSize делает width/height
    // размером ИМЕННО содержимого - именно то число пикселей, на которое
    // рассчитывает сама доска.
    useContentSize: true,
  };
}
