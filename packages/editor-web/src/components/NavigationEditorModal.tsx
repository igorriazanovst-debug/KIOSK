...
import { buildMultiFloorGraph } from '../utils/navigation/multiFloor';
...
const NavigationEditorModal: React.FC<NavigationEditorModalProps> = ({
  ...
  const [debugLog, setDebugLog] = useState<string>(''); // NAV_FLOOR_SVG_DEBUG
  ...
  // ── Проверка доступности помещений ──────────────────────────────────
  const accessIssues = useMemo(() => {
    type Issue = { roomId: string; title: string; floorId: string; floorName: string; status: 'unreachable' | 'no-entry' };
    const issues: Issue[] = [];
    let graph;
    try { graph = buildMultiFloorGraph(navData, {}); } catch { return issues; }
    const termNodes: string[] = Object.values(graph.terminalNode);
    const reachable = new Set<string>();
    if (termNodes.length > 0) {
      const q: string[] = [...termNodes];
      termNodes.forEach((n) => reachable.add(n));
      while (q.length) {
        const u = q.shift()!;
    ...
    for (const f of navData.floors) {
      for (const r of f.rooms) {
        const poiNode = graph.roomPoiNode[r.id];
        const hasEntry = !!r.entry;
        let status: Issue['status'] | null = null;
        if (termNodes.length === 0) {
          if (!hasEntry) status = 'no-entry';
        } else if (!poiNode || !reachable.has(poiNode)) {
          status = 'unreachable';
        } else if (!hasEntry) {
          status = 'no-entry';
        }
        if (status) issues.push({ roomId: r.id, title: r.title || r.id, floorId: f.id, floorName: f.name, status });
      }
    }
    return issues;
  }, [navData]);
  ...
  useEffect(() => {
    ...
    const svgLen = activeFloor?.svgContent?.length ?? 0;
    const hasRef = !!svgHostRef.current;
    setDebugLog(prev => prev + `\n[effect] floorId=${floorId} svgLen=${svgLen} hasRef=${hasRef} ts=${Date.now()}`);
    if (!activeFloor?.svgContent || !svgHostRef.current) {
      setDebugLog(prev => prev + `\n[effect] EARLY RETURN svgContent=${!!activeFloor?.svgContent} ref=${!!svgHostRef.current}`);
      return;
    }
    ...
    const parseSvgAndFit = () => {
      ...
      setDebugLog(prev => prev + `\n[rAF] svgFound=${!!svg} stageW=${stageRect?.width?.toFixed(0)} stageH=${stageRect?.height?.toFixed(0)}`);
      ...
      setDebugLog(prev => prev + `\n[fit] vb=${width}x${height} stageW=${rect?.width?.toFixed(0)} stageH=${rect?.height?.toFixed(0)}`);
      if (!rect || rect.width < 10 || rect.height < 10) {
        // Stage ещё не смонтирован с размерами — повторяем через 50ms
        setDebugLog(prev => prev + '\n[fit] stage too small, retry 50ms');
  ...
  function onStageMouseDown(e: React.MouseEvent) {
    if (tool === 'move' && e.button === 0 && activeFloor) {
      const pt = toFloorCoords(e.clientX, e.clientY);
      const hit = hitTestMove(activeFloor, pt);
      if (hit) { dragRef.current = hit; didPanRef.current = false; e.preventDefault(); return; }
      return;
    }
  ...
  function onStageMouseMove(e: React.MouseEvent) {
    if (tool === 'move' && dragRef.current) {
      const pt = snapToGrid(toFloorCoords(e.clientX, e.clientY));
      moveObject(dragRef.current, pt);
      return;
    }
  ...
  function fitToStage(vbWidth: number, vbHeight: number) {
    ...
    setDebugLog(prev => prev + `\n[fitToStage] z=${z.toFixed(4)} pan=${newPan.x.toFixed(0)},${newPan.y.toFixed(0)}`);
  }
  ...
  // ── Перемещение точечных объектов (tool=move) ───────────────────────
  const dragRef = useRef<null | { type: 'entry' | 'poi' | 'service' | 'terminal'; id: string }>(null);

  function hitTestMove(floor: FloorData, pt: Point): { type: 'entry' | 'poi' | 'service' | 'terminal'; id: string } | null {
    const thr = 22 / zoomRef.current;
    let best: { type: 'entry' | 'poi' | 'service' | 'terminal'; id: string } | null = null;
    let bestD = thr;
    const consider = (x: number, y: number, type: 'entry' | 'poi' | 'service' | 'terminal', id: string) => {
      const d = Math.hypot(x - pt.x, y - pt.y);
      if (d <= bestD) { bestD = d; best = { type, id }; }
    };
    for (const r of floor.rooms) {
      if (r.poi) consider(r.poi.x, r.poi.y, 'poi', r.id);
      if (r.entry) consider(r.entry.x, r.entry.y, 'entry', r.id);
    }
    for (const sp of floor.service) consider(sp.x, sp.y, 'service', sp.id);
    for (const t of floor.terminals) consider(t.x, t.y, 'terminal', t.id);
    return best;
  }

  function moveObject(d: { type: 'entry' | 'poi' | 'service' | 'terminal'; id: string }, pt: Point) {
    setNavData((prev) => ({
      ...prev,
      floors: prev.floors.map((f) => {
        if (f.id !== activeFloorId) return f;
        if (d.type === 'poi') return { ...f, rooms: f.rooms.map((r) => r.id === d.id ? { ...r, poi: { x: pt.x, y: pt.y } } : r) };
        if (d.type === 'entry') return { ...f, rooms: f.rooms.map((r) => r.id === d.id ? { ...r, entry: { x: pt.x, y: pt.y } } : r) };
        if (d.type === 'service') return { ...f, service: f.service.map((s) => s.id === d.id ? { ...s, x: pt.x, y: pt.y } : s) };
        if (d.type === 'terminal') return { ...f, terminals: f.terminals.map((t) => t.id === d.id ? { ...t, x: pt.x, y: pt.y } : t) };
        return f;
      }),
    }));
  }
  ...
  function handleUploadPlan(file: File) {
    ...
    reader.onload = (ev) => {
      ...
      // MULTILINE_SVG_FIX + NAV_SVG_SIZE_FIX
      // 1. Убираем width/height в физических единицах (mm, cm и т.д.)
      // 2. Вставляем width/height из viewBox в пикселях — иначе SVG рендерится 0x0
      const stripped = text
        .replace(/(<svg[\s\S]*?)\s+width="[^"]*"/i, '$1')
        .replace(/(<svg[\s\S]*?)\s+height="[^"]*"/i, '$1');
      const vbMatch = stripped.match(/viewBox="[\d.\s,]*?([\d.]+)\s+([\d.]+)"/i);
      const vbW = vbMatch ? parseFloat(vbMatch[1]) : 1000;
      const vbH = vbMatch ? parseFloat(vbMatch[2]) : 1000;
      const normalized = stripped.replace(/<svg([\s\S]*?)>/i,
        `<svg$1 width="${vbW}" height="${vbH}">`);
      setDebugLog(prev => prev + `\n[upload] activeFloor.id=${activeFloor.id} normalizedLen=${normalized.length} hasSvgTag=${normalized.includes('<svg')}`);
      // NAV_SVG_ATTRS_DEBUG
      const _svgTagMatch = normalized.match(/<svg([^>]*)>/i);
      const _svgAttrs = _svgTagMatch ? _svgTagMatch[1].replace(/\s+/g,' ').trim().slice(0,400) : 'no svg tag';
      setDebugLog(prev => prev + `\n[upload] id=${activeFloor.id} len=${normalized.length}\n[svgTag] ${_svgAttrs}`);
  ...
  const hintByTool: Record<Tool, string> = {
    ...
    move: 'Тяните точку входа, POI, лестницу, лифт или терминал, чтобы переместить.',
        ...
        <div className="nav-editor-body">
          <div className="nav-editor-side">
            <div className="scroll">
              ...
              <h4 style={{ color: accessIssues.length ? '#e0a020' : '#5a8' }}>
                Доступность {accessIssues.length ? `(проблем: ${accessIssues.length})` : '(OK)'}
              </h4>
              {accessIssues.length === 0 && (
                <div className="nav-editor-info">Все помещения достижимы и имеют точку входа.</div>
              )}
              {accessIssues.map((a) => (
                <div
                  key={a.roomId}
                  onClick={() => setActiveFloorId(a.floorId)}
                  title="Перейти на этаж помещения"
                  style={{
                    cursor: 'pointer', fontSize: 11, padding: '4px 6px', marginBottom: 3,
                    borderRadius: 3, background: '#2a2a2a',
                    borderLeft: `3px solid ${a.status === 'unreachable' ? '#e07a7a' : '#e0a020'}`,
                  }}
                >
                  <div style={{ color: a.status === 'unreachable' ? '#e07a7a' : '#e0a020' }}>
                    {a.title} <span style={{ color: '#777' }}>· {a.floorName}</span>
                  </div>
                  <div style={{ fontSize: 9, color: '#999', marginTop: 1 }}>
                    {a.status === 'unreachable' ? 'нет пути от терминала — проверьте коридоры/точку входа' : 'нет точки входа (entry)'}
                  </div>
                </div>
              ))}
          ...
          <div
            ...
            style={{ cursor: isPanningRef.current ? 'grabbing' : tool === 'pan' ? 'grab' : tool === 'move' ? 'move' : 'crosshair', userSelect: 'none' }}
          >
            {!activeFloor && (
            ...
            {/* NAV_FLOOR_SVG_DEBUG */}
            {debugLog && (
              <div style={{position:'absolute',bottom:8,left:8,right:8,zIndex:9999,background:'rgba(0,0,0,0.92)',borderRadius:6,padding:8}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4}}>
                  <span style={{color:'#f90',fontSize:11,fontWeight:'bold'}}>SVG Debug Log</span>
                  <button style={{fontSize:10,padding:'1px 6px',background:'#333',color:'#ccc',border:'1px solid #555',borderRadius:3,cursor:'pointer'}} onClick={()=>setDebugLog('')}>✕ clear</button>
                </div>
                <textarea readOnly value={debugLog} style={{width:'100%',height:120,fontSize:10,fontFamily:'monospace',background:'#111',color:'#8f8',border:'1px solid #333',borderRadius:3,resize:'vertical',boxSizing:'border-box'}} />
              </div>
            )}
