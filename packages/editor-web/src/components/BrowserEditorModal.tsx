...
const PageTreeItem: React.FC<PageTreeItemProps> = ({
  ...
  return (
    <div>
      ...
      >
        <span className="btree-icon">{children.length > 0 ? '📁' : '📄'}</span>
        <span className="btree-title">{page.title || 'Без названия'}</span>
        <div className="btree-actions">
          <button title="Добавить подстраницу" onClick={(e) => { e.stopPropagation(); onAddChild(page.id); }}>+</button>
          <button title="Удалить" onClick={(e) => { e.stopPropagation(); onDelete(page.id); }}>🗑</button>
        </div>
      </div>
      {children.map(child => (
        <PageTreeItem
...
const ImageInsertDialog: React.FC<ImageInsertDialogProps> = ({ onInsert, onClose, projectId }) => {
  ...
  const handleUpload = async (file: File) => {
    if (!projectId) { alert('Проект не сохранён'); return; }
    setUploading(true);
    try {
  ...
  return (
    <div className="binsert-dialog" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="binsert-tabs">
        ...
        <button className={tab === 'upload' ? 'active' : ''} onClick={() => setTab('upload')}>Загрузить</button>
      </div>
      {tab === 'url' && (
      ...
      <div className="binsert-options">
        <label>Ширина: <input type="text" value={width} onChange={(e) => setWidth(e.target.value)}
      ...
      {tab === 'url' && (
        <div className="binsert-footer">
          <button className="bbtn-primary" onClick={insertUrl} disabled={!url}>Вставить</button>
        </div>
      )}
    </div>
  );
...
<div style="display:flex;justify-content:center;gap:8px;margin-top:6px;">
  ...
  return (
    <div className="binsert-dialog" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="binsert-tabs">
        <button className={type === 'grid' ? 'active' : ''} onClick={() => setType('grid')}>Сетка</button>
        <button className={type === 'slider' ? 'active' : ''} onClick={() => setType('slider')}>Слайдер</button>
      </div>
      <div className="binsert-body">
        <div style={{ display: 'flex', gap: '6px', marginBottom: '8px' }}>
          <input placeholder="URL изображения" value={urlInput}
          ...
          {images.map((src, i) => (
            <div key={i} style={{ position: 'relative' }}>
              ...
              <button onClick={() => setImages(prev => prev.filter((_, j) => j !== i))}
                style={{ position: 'absolute', top: 0, right: 0, background: 'rgba(0,0,0,0.6)', color: '#fff', border: 'none', borderRadius: '50%', width: '16px', height: '16px', cursor: 'pointer', fontSize: '10px', padding: 0 }}>✕</button>
            </div>
          ))}
        </div>
        {type === 'grid' && (
          <div style={{ marginTop: '8px' }}>
            <label style={{ color: '#aaa', fontSize: '12px' }}>Колонок: </label>
...
const LinkInsertDialog: React.FC<LinkInsertDialogProps> = ({ pages, onInsert, onClose }) => {
  ...
  const [label, setLabel] = useState('Нажмите здесь');
  ...
  return (
    <div className="binsert-dialog" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="binsert-tabs">
        <button className={tab === 'page' ? 'active' : ''} onClick={() => setTab('page')}>Страница браузера</button>
        <button className={tab === 'slide' ? 'active' : ''} onClick={() => setTab('slide')}>Слайд проекта</button>
      </div>
      <div className="binsert-body">
        <div style={{ marginBottom: '10px' }}>
          <label style={{ color: '#aaa', fontSize: '12px', display: 'block', marginBottom: '4px' }}>Текст ссылки</label>
          <input className="binsert-input" value={label} onChange={(e) => setLabel(e.target.value)} />
        </div>
        {tab === 'page' && (
          <div>
            <label style={{ color: '#aaa', fontSize: '12px', display: 'block', marginBottom: '4px' }}>Страница</label>
        ...
        {tab === 'slide' && (
          <div>
            <label style={{ color: '#aaa', fontSize: '12px', display: 'block', marginBottom: '4px' }}>Слайд</label>
            ...
            <div style={{ fontSize: '11px', color: '#666', marginTop: '4px' }}>
              * Навигация по слайдам работает только в Player
            </div>
          </div>
        )}
      </div>
...
const BrowserEditorModal: React.FC<BrowserEditorModalProps> = ({
  ...
  const [pages, setPages] = useState<BrowserPage[]>(
    initialPages.length > 0
      ? initialPages
      : [{ id: generateId(), title: 'Главная', htmlContent: '' }]
  );
  ...
  const addPage = () => {
    const np: BrowserPage = { id: generateId(), title: 'Новая страница', htmlContent: '' };
    setPages(prev => [...prev, np]);
    saveCurrentAndSelect(np.id);
  };

  const addChildPage = (parentId: string) => {
    const np: BrowserPage = { id: generateId(), title: 'Подстраница', htmlContent: '', parentId };
    setPages(prev => [...prev, np]);
    saveCurrentAndSelect(np.id);
  };
    ...
    >
      <div className="bmodal">
        <div className="bmodal-header">
          ...
          <button className="bmodal-close" onClick={onClose}>✕</button>
        </div>
          ...
          <div className="bmodal-body">
            <div className="bmodal-tree">
              ...
              <div className="btree-list">
                ...
                {pages.length === 0 && (
                  <div className="btree-empty">Нет страниц. Нажмите «+ Добавить»</div>
                )}
              </div>
            </div>
              ...
              {selectedPage ? (
                <>
                  ...
                  <div className="bpage-toolbar">
                    ...
                    <span className="btoolbar-sep" />
                    <label title="Цвет текста" style={{ display: 'flex', alignItems: 'center', gap: '2px', cursor: 'pointer' }}>
                    ...
                    <label title="Цвет подсветки" style={{ display: 'flex', alignItems: 'center', gap: '2px', cursor: 'pointer' }}>
                ...
                <div className="bpage-empty">Выберите страницу слева</div>
              )}
            </div>
          </div>
        )}
        ...
        {activeTab === 'settings' && (
          <div className="bmodal-settings">
            ...
            {/* Позиция меню — не используется в режиме связанной пары */}
        ...
        <div className="bmodal-footer">
          ...
          <button className="bbtn-primary" onClick={handleSave}>Сохранить</button>
        </div>
      </div>
    </div>
  );
