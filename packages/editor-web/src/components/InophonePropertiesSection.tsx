      ...
      <div className="property-field">
        ...
        {INOPHONE_LANGUAGES.filter((lang) => lang.code !== interfaceLanguage).map((lang) => (
          <label key={lang.code} style={{ display: 'block', fontWeight: 400 }}>
            ...
            />{' '}
            {lang.russianName} — {lang.nativeName}
            {/* Приписки про озвучку здесь БОЛЬШЕ НЕТ, и это осознанно.
                Стояло «(озвучка синтезом недоступна)» у языков без
                `speechTag` — тега системного голоса. Редактор работает в
                браузере, пакета контента рядом нет, и сказать, записано ли
                произношение, он не может в принципе. Утверждение разошлось с
                правдой, как только башкирский озвучили: тега у него нет, а
                444 записи в пакете есть. Что записано, а что нет, показывает
                плеер — на экране настроек и в сведениях о пакете, где эти
                сведения берутся у самого пакета. */}
          </label>
        ))}
      </div>
