...
const WidgetLibrary: React.FC = () => {
  ...
  const widgetTypes = [
    ...
    ...(isAlphabetAllowed ? [{
      type: ALPHABET_WIDGET_TYPE,
      name: 'АзбукоСлов',
      // Type — та же иконка-литера, что у текстового виджета. Взята не от
      // лени: lucide-react здесь версии 0.263.1, и брать из неё иконку,
      // которой в этой версии может не быть, — способ уронить импорт всего
      // редактора ради значка
      icon: Type,
      defaultProps: ALPHABET_DEFAULT_PROPS,
      defaultSize: ALPHABET_DEFAULT_SIZE
    }] : [])
  ];
