...
export class AdminController {
  ...
  static async createLicense(req: Request, res: Response) {
    ...
    const { PLANS } = await import('@kiosk/shared');
    const planConfig = PLANS[plan.toLowerCase() as 'basic' | 'pro' | 'max'];
    
    // Создать лицензию
    const license = await LicenseService.createLicense({
      organizationId,
      plan,
      seatsEditor: seatsEditor || planConfig.maxEditorDevices,
      seatsPlayer: seatsPlayer || planConfig.maxPlayerDevices,
      validFrom: new Date(),
